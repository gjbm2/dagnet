...
This directory provides playbooks, reference docs, and scripts so that an AI coding agent (or human) can create, edit, and maintain DagNet conversion graphs in the `nous-conversion` data repo following a standard branch-based workflow.
...
└── scripts/
    ├── new-branch.sh           # Create and checkout a feature branch in nous-conversion
...
The production data repo is cloned at `nous-conversion/` (workspace root). It is **git-ignored** — never commit it to the public dagnet repo.

```
nous-conversion/                # git@github.com:gjbm2/nous-conversion.git
...
- Any staged files under `nous-conversion/`
