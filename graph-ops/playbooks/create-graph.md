...
**Prerequisite**: You are on a feature branch in `nous-conversion` (see [branch-workflow.md](branch-workflow.md)).
...
For each unique analytics event in the funnel, create `nous-conversion/events/<id>.yaml`.
...
**Reuse existing events** where possible — check `nous-conversion/events-index.yaml` first.

---

## Step 3: Create Node Files

For each graph node, create `nous-conversion/nodes/<id>.yaml`.
...
Create `nous-conversion/graphs/<graph-name>.json`.
...
For each edge that will carry data, create `nous-conversion/parameters/<id>.yaml`.
