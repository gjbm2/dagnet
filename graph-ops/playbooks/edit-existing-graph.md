...
How to modify an existing graph in the data repo.
...
## Auto-Arrange Layout (REQUIRED after every edit)

After any structural change (adding/removing nodes or edges), re-run a layout pass on the entire graph. See [create-graph.md](create-graph.md) Step 7 for the full rules. Quick summary:

1. **All edges must flow left-to-right** (source x < target x). No same-x, no backward.
2. **One input face (left), one output face (right)** per node — inputs and outputs must NOT share a side.
3. **Layered x-positions** — group by topological depth, space layers ~600px apart.
4. **Vertical separation by branch** — keep parallel paths well apart so edges don't cross. Main/happy path near y=0, branches above and below.
5. **Abandon/failure nodes below** their branch path (higher y), at least one layer past their rightmost feeder.
6. **No duplicate positions** — every node must have a unique (x, y).

This is not optional. Skipping the layout pass results in backward edges, overlapping nodes, and unreadable graphs.
...
**Layout**
- [ ] All edges flow left-to-right (source x < target x strictly)
...
- [ ] Abandon/failure nodes placed below their branch
...
grep -rl "<node-id>" <data-dir>/graphs/

# Find which parameters reference a node
grep -rl "<node-id>" <data-dir>/parameters/

# Find all edges from a specific node (by UUID)
grep "<node-uuid>" <data-dir>/graphs/<graph>.json
