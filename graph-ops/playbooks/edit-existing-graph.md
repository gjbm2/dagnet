...
How to modify an existing graph in `<private-repo>`.
...
grep -rl "<node-id>" <private-repo>/graphs/

# Find which parameters reference a node
grep -rl "<node-id>" <private-repo>/parameters/

# Find all edges from a specific node (by UUID)
grep "<node-uuid>" <private-repo>/graphs/<graph>.json
...
" <private-repo>/graphs/<graph>.json
