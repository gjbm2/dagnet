...
Standard git workflow for developing graph changes in `nous-conversion`.
...
2. Clone or switch to the `nous-conversion` repo at the feature branch
...
1. Create a PR on GitHub (`gjbm2/nous-conversion`)
...
| Switch to main      | `cd nous-conversion && git checkout main && git pull`       |
| List branches       | `cd nous-conversion && git branch -a`                       |
| Delete local branch | `cd nous-conversion && git branch -d feature/<name>`        |
