...
How to create individual entity files (node, event, parameter, context) in the data repo (`<data-dir>` below — see `graph-ops/repos.conf` for the actual directory name).
...
grep -i "<event-name>" <data-dir>/events-index.yaml

# Check existing nodes
grep -i "<node-name>" <data-dir>/nodes-index.yaml

# Check existing parameters
grep -i "<param-name>" <data-dir>/parameters-index.yaml
```
...
**User properties** (set via Segment `identify()`, e.g. `onboardingSegment`): add `property_type: user` or the filter will silently fail — the DAS adapter defaults to `subprop_type: "event"` which won't match user traits:

```yaml
amplitude_filters:
  - property: onboardingSegment
    property_type: user
    operator: is
    values:
      - can_offer_energy_retention
```

### Index Entry

Add to `<data-dir>/events-index.yaml` under `events:`:
...
Add to `<data-dir>/nodes-index.yaml` under `nodes:`:
...
**File**: `<data-dir>/parameters/<id>.yaml`
...
Add to `<data-dir>/parameters-index.yaml` under `parameters:`:
...
> **Critical**: `name` is required. Without it the parameter shows as red/broken ("Not in registry") in the editor even if the file exists and the YAML is valid.
...
Check `<data-dir>/contexts-index.yaml` — current contexts are:
...
Add to `<data-dir>/contexts-index.yaml` under `contexts:`.
