...
### 1. `conditionalColours.ts` (Line ~90)
```typescript
...
2. Add a complementary conditional_p to each sibling with:
   ...
   - **Same colour** (visual grouping across siblings)
...
System AUTO-ADDS to A→C and A→D:
  A→C.conditional_p: { condition: "visited(promo)", p: 0.15, colour: <same> }
  A→D.conditional_p: { condition: "visited(promo)", p: 0.05, colour: <same> }
  (0.8 + 0.15 + 0.05 = 1.0)
```
...
3. Logic:
   ...
   addComplementaryConditionals(edge: GraphEdge, newCondition: ConditionalProbability) {
     ...
     const colour = getOrAssignConditionalColour(newCondition.condition);
     ...
     for (const sibling of siblings) {
       ...
       if (!existing && !sibling.condition_overridden) {
         ...
         sibling.display.conditional_colour = colour;
       }
     }
...
#### 3.2 Colour Picker (LOST FEATURE)

**What It Was:**
- UI control in properties panel for conditional probabilities
- Allowed user to select colour for a condition
- Colour would be set on `edge.display.conditional_colour`
- Colour would propagate to all sibling edges with same condition
- Used for visual grouping in graph view

**Where It Was:**
- Likely in `ConditionalProbabilitiesSection.tsx` (old UI component)
- ColourPicker component from UI library

**Where It Should Go:**
- `ConditionalProbabilityEditor.tsx` - add colour picker next to condition field
- Or in properties panel, at the edge level (one colour picker per unique condition across all edge's conditional_ps)

**How To Restore:**
```typescript
// In ConditionalProbabilityEditor.tsx
<div className="conditional-colour-picker">
  <label>Condition Colour</label>
  <ColourPicker
    value={condition.display?.conditional_colour || getNextAvailableColour()}
    onChange={(colour) => {
      // Update this edge
      updateCondition(index, { 
        display: { conditional_colour: colour } 
      });
      
      // Update all siblings with same condition (via UpdateManager)
      updateManager.propagateConditionalColour(
        edgeId, 
        condition.condition, 
        colour
      );
...
updateManager.propagateConditionalColour(edgeId, condition, colour);
```
...
class UpdateManager {
  ...
  graphToGraph: {
    ...
    propagateConditionalColour(edgeId, condition, colour);
...
2. ⚠️ Extract useful logic BEFORE removal:
   ...
   - Line ~XXX: Colour picker component
