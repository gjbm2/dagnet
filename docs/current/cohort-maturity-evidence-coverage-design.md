...
### 5.2 Atom 2 — unify the display path across window and active
...
**Goal**: most of this machinery is a **no-op for window queries** — that's the test of correctness. The no-op must emerge as natural degeneracy from the same composition pattern, not via `if is_window:` branches. Minimum code surface, minimum test span.
...
#### Runtime delta — minimal

- `_build_active_selected_a_clock_evidence_from_runtime` is invoked unconditionally. The carrier-chain composition step short-circuits for identity carrier (population_root == denominator_node) — the carrier surface is built from the X-rooted subject primitive's `n`. The subject-chain composition runs unchanged. The carrier backmap, when invoked, is identity in identity-carrier mode (no shift).
- The row builder drops the `is_active_carrier` branch on the evidence side. Reads `aggregate_by_tau` for both modes.
- The frame-derived `engine_cohorts.obs_x/obs_y` forward-fill in `_project_runtime_rows` is removed for evidence display. `engine_cohorts` continues to feed the reducer's identity-carrier prefix until atom 3 retires it.
- The reducer's selected-prefix path is invoked for both modes since `SelectedAClockEvidence` is now built for both. The reducer's existing identity-carrier branch consumes it.

**What atom 2 does not change**: the canonical semantics; the conditioning binding (still local-clock for window, propagated for cohort); the carrier/subject composition algebra; the model overlay; the completeness machinery; public scalar moments; the cache identity rules.

**Risk**: window-mode tests have implicit dependencies on the legacy forward-fill. Single-hop window numerical content should be identical (one primitive, one edge, max-flow trivially yields `k`; denominator from the same primitive's `n`). Multi-hop window numerical content may differ from the legacy `compose_path_maturity_frames` path — the legacy path's window-mode composition was never formally pinned by the canonical doc; the new path's max-flow on the calendar-date axis is the principled mixed-cohort interpretation. Drift surfaces case-by-case for review before atom 2 lands.

**Test surface**:
- All atom-1 tests stay green.
- Single-hop window display tests stay green at the numerical level.
- New pinning test: window single-hop is the chain-of-length-1 degeneracy of the unified composition (running the same composition on a one-edge chain yields the equivalent result).
- New pinning test: identity carrier is the chain-of-length-0 degeneracy — the carrier surface reads the X-rooted subject primitive's `n` and the carrier backmap is identity.
- The outside-in suite's window-mode and active-cohort scenarios re-run without regressions; multi-hop window drift is investigated case-by-case before the atom lands.

### 5.3 Atom 3 — retire the frame-derived evidence layer

**Scope**: once the row builder no longer reads from `engine_cohorts.obs_x/obs_y`, shrink `build_cohort_evidence_from_frames` to its remaining responsibilities (cohort_list materialisation, epoch-boundary derivation, `a_pop` plumbing, evidence-superset metadata) and remove the forward-fill machinery and the `is_active_carrier` parameter that today gates whether the prefix is zeroed.

**Goal**: zero parallel evidence surfaces. One read from the runtime, one prefix object consumed by the reducer and the row builder, one canonical source for chart evidence.

**Test surface**: the active-cohort and window-mode regression nets continue to apply. No new tests beyond pinning the absence of the legacy code paths (e.g. asserting `engine_cohorts.obs_x` is empty or removed from the runtime contract).
...
4. **Window-mode parity in atom 2**. The cleanest test is to compute the legacy and primitive-bound evidence in parallel for a fixed window-mode fixture and assert numerical equivalence. For single-hop window, the values should be identical. For multi-hop window, parity with the legacy path is **not** the goal — the legacy path is suspect (it reads frame `dp.x`/`dp.y` which may not faithfully reflect topology composition for chains). Drift in multi-hop must be diagnosed before atom 2 lands; the resolution may be that the new path is correct and a legacy multi-hop chart bug existed undetected, or that a parity bug in the new composition needs fixing.
5. **`a_pop` in window mode**. Today `a_pop` is sourced from the frame's `dp.a` for window and from `_root_window_carrier_n_by_anchor_day` for active. Atom 2 should not change the `a_pop` derivation — only the evidence display. The `a_pop` plumbing remains in `compute_cohort_maturity_rows_v3` and is unaffected.
...
- The frame-derived `engine_cohorts.obs_x/obs_y` forward-fill (post-atom-3) is no longer reachable from the row builder.
