...
4. Cohort_maturity row reducer: stopgap fix for the `_runtime_completeness` IndexError at `cohort_forecast_v3.py:1416`. The crash is that the internal `_runtime_request_cdf_draws` call composes the CDF only to `horizon=max_tau` (the row count), while `cohort_eval_ages` carries per-Cohort frontier τ that can exceed `max_tau` under the new `compute_extent`-driven engine. Widen the internal compose horizon inside `_runtime_completeness` to cover the eval points — compose to `max(max_tau, max(cohort_eval_ages) + 1)`. The `completeness` / `completeness_sd` row fields, the FE forward at `graphComputeClient.ts:500`, the CF endpoint's read of `last_row.get("completeness")` at `api_handlers.py:2042-2043`, and the param-pack parity test at `test_cohort_factorised_outside_in.py:1429-1471` all stay as-is. They are retired in Phase 5e of the 73q plan once the dedicated scalar reducer replaces this co-production. This is a deliberate stopgap, scoped to the now-work / Phase 5e gap.
