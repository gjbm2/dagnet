...
| 51 | [51-subset-conditioning-double-count-correction.md](51-subset-conditioning-double-count-correction.md) | **Problem statement + proposal** (20-Apr-26) | Every engine that uses the promoted posterior `Beta(α₀, β₀)` as a prior and multiplies by a subset likelihood double-counts the subset (it is already inside the aggregate fit). Overstatement scales with subset share: ~5% at 5%, ~33% at half, 50% at full. Callsites: cohort-maturity v3 Beta update, CF Class B (doc 50), cohort-mode forecast conditioning. Proposes a shared pro-rata shrinkage helper — discount aggregate prior's equivalent strength by `N_subset / N_total` before updating. Display-time correction, not a hierarchical re-fit | Implementation not started. Listed in programme.md as P1.12 under Tier-1 |
