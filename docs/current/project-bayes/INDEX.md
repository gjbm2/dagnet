...
| 71 | [71-shared-cf-runtime-detailed-execution-plan.md](71-shared-cf-runtime-detailed-execution-plan.md) | **Active implementation plan** (23-Apr-26) | Operational companion to doc 66. Adds execution discipline, residue inventory, parity points, commit boundaries, and failure-mode guards for the shared-runtime refactor | Use as the detailed staging note for doc 66 execution and rollback-safe implementation discipline |
| 72 | [72-fe-cli-conditioned-forecast-parity-fix-plan.md](72-fe-cli-conditioned-forecast-parity-fix-plan.md) | **Historical implementation note** (24-Apr-26) | Narrow FE/CLI parity investigation record retained for provenance. The current execution programme lives in docs 73a–c (renumbered from the original 75/73/76) | Do not use as the primary execution note |
| 73a | [73a-scenario-param-pack-and-cf-supersession-plan.md](73a-scenario-param-pack-and-cf-supersession-plan.md) | **Active implementation plan** (was doc 75 — renumbered 25-Apr-26) | Tactical CF parity work: per-scenario CF supersession, pack contract + extractor/compositor fidelity, CF observability, post-CF upsert race, CLI/FE alignment, verification rollup. Per-scenario `model_vars[]` delivery and legacy-path cleanup were retired from this doc on 25-Apr-26 and now live inside doc 73b's bundled switchover so each commit produces an observable diff | Current execution note for tactical CF/pack/supersession work |
| 73b | [73b-be-topo-removal-and-forecast-state-separation-plan.md](73b-be-topo-removal-and-forecast-state-separation-plan.md) | **Active implementation plan** (was doc 73 — renumbered 25-Apr-26) | Architectural plan: source/promoted/current-answer layering, removal of `manual` from source taxonomy, analytic semantic transition, promoted surface (`p.forecast.*`) design and writer, the bundled switchover that combines per-scenario delivery + promoted writer + first BE consumer migration in one stage, the remaining consumer migrations, and removal of `_posteriorSlices` / `reprojectPosteriorForDsl` legacy paths | Current execution note for source/promoted/current-answer separation work |
| 73b-B | [73b-appendix-b-data-flow-and-interfaces.md](73b-appendix-b-data-flow-and-interfaces.md) | **Active reference** (companion to 73b — extracted 27-Apr-26) | Data-flow diagrams and interface contracts (I1–I17) for the post-73b forecast stack: source provenance (B.1), per-edge layered model L1–L5 (B.2), per-scenario request graph (B.3), BE analyse dispatch full surface (B.4). Lives in `project-bayes/` while 73b is in flight; promoted to `docs/current/codebase/` after 73b acceptance per 73b §11.1 closing item | Read alongside 73b for any work on the layered contract or any of the labelled interfaces |
| 73c | [73c-cf-staged-test-strategy.md](73c-cf-staged-test-strategy.md) | **Sidecar test strategy** (was doc 76 — renumbered 25-Apr-26) | Sidecar to doc 73a. Per-layer test strategy reasoned from first principles: enumerates concrete failure modes per layer, groups into minimum test sets, names artefacts. Layer A and Layer C complete; B/D/E/F/G/H pending | Read alongside doc 73a when implementing each stage |
| 74 | [74-doc-72-scope-correction.md](74-doc-72-scope-correction.md) | **Companion scope note** (24-Apr-26) | Short scoping note recorded after the reversion. Keeps doc 72's useful warning about crossing model state with query-scoped answers while explicitly de-scoping broader clean-up framing | Read alongside doc 73a, not as a standalone implementation plan |
