...
| 63 | [63-investigation-tracker-mcp-spec.md](63-investigation-tracker-mcp-spec.md) | **Proposed** (22-Apr-26) | Spec for a tiny MCP-backed investigation tracker for Bayes diagnostic work. Keeps run purpose, allowed conclusions, blocker classification, and next-step state in a structured YAML source of truth next to `20-open-issues-register.md`, with the markdown register rendered as the human view | Not implemented yet. Intended to replace markdown-only run tracking for agent-driven diagnostic work |
