...
| 59 | [59-cohort-window-forecast-implementation-scheme.md](59-cohort-window-forecast-implementation-scheme.md) | **Proposed / review-ready** (21-Apr-26) | Target implementation contract derived from `COHORT_ANALYSIS_NUMERATOR_DENOMINATOR_SEMANTICS.md`. Defines one naturally degenerating runtime template (`population_root`, `carrier_to_x`, `subject_span`, `numerator_representation`, `admission_policy`, `p_conditioning_evidence`), object-by-object prior/evidence rules, a narrow flagged path for direct `cohort()` evidence to condition `p`, and the surface-ownership contract for a later delta analysis against live code. Framed as document 2 of the three-doc peer-review pack | No delta analysis yet. Intended as the target-state scheme to compare the current forecast stack against before code changes widen further |
| 60 | [60-forecast-adaptation-programme.md](60-forecast-adaptation-programme.md) | **Active / review-ready** (21-Apr-26) | Delivery-grade implementation plan for the forecast adaptation workstream. States required reading, settled design decisions, verified deltas, file ownership, work packages, test homes, PR boundaries, and final acceptance criteria across whole-graph CF, scoped CF, trajectory, summary, funnel, and graph-state consumers. Framed as document 3 of the peer-review pack, with review prompts and reading order | Implementation not started. This is the working execution plan for the adaptation refactor |
