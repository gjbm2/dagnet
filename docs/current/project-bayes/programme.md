...
- **Forecast-stack residual v1/v2 coupling — migrate live callers off
  legacy helpers** (doc 56) — P2.16. Prerequisite for v1/v2 deletion
  (absorbs half of P2.10), fixes the κ=20 prior defect that surfaced
  in doc 50 truth-parity, and unblocks docs 47 / 52 / 55 consumer work.
...
- **Surprise gauge rework — thin projection of
  `compute_forecast_summary`** (doc 55) — P2.18. Removes FE local
  compute and the analytic fallback; needs two scalar-field
  additions to `ForecastSummary`. Awaits approval.
- **Forecast function naming rename (`compute_forecast_summary` /
  `compute_forecast_trajectory` + `completeness_conditioned` /
  `_unconditioned`)** (doc 56-naming) — P3.25. Should land before
  doc 52 / 55 / 47 implementation so new code uses final
  identifiers.
...
| P2.18 | **Surprise gauge rework — thin projection of `compute_forecast_summary`** (doc 55, 20-Apr-26). Scope contracts to two variables: `p` (observed aggregate vs unconditioned posterior-predictive rate) and `completeness` (unconditioned vs IS-conditioned posterior completeness). Removes FE local compute (`buildSurpriseGaugeResult` + helpers), removes BE analytic fallback, stops reading `analytic_be` as a privileged source. Low IS ESS degrades gracefully with a warning icon rather than suppressing display. μ / σ / onset descoped. | [doc 55](55-surprise-gauge-rework.md). Requires two scalar-field additions to `ForecastSummary` (`completeness_unconditioned` / `_sd`, `pp_rate_unconditioned` / `_sd`), both computed internally today. Depends on doc 56-naming (P3.25) landing first so new code writes to the renamed return type | Awaits approval. Sequence after P3.25 (rename) and P2.16 (v1/v2 retirement). Supersedes `surprise-gauge-design.md` §5.1 / §5.2 / §6.1 on implementation |
| P2.19 | **CF eligibility — topological degradation when Bayes fit is incomplete** (doc 57, 21-Apr-26). Replaces the abandoned doc 56 Phase 4.5 κ=20 band-aid. Rule: CF runs its sweep only on edges where the edge itself has a Bayesian fit AND every upstream edge on every path from the anchor to it is also Bayesian-fit. Otherwise the edge gets a degraded analytic view (point estimate + binomial-SE bands) with `cf_source: "analytic-degraded"` and `cf_degradation_reason` provenance so FE can render distinctly. Contamination is topological: one non-Bayesian edge invalidates CF conditioning for the entire subgraph below it. | [doc 57](57-cf-eligibility-topological-degradation.md). Current fallback at `build_span_params` / `span_adapter` uses κ=20 regardless, producing nonsense on non-Bayesian edges. Doc 56 §8 Phase 4.5 and §11.2 to be updated to point here on implementation | Phased: (1) implement eligibility helper + unit tests on small graphs; (2) wire into CF handler + v3; (3) extend response schema with `cf_source` / `cf_degradation_reason`; (4) re-run `cf-truth-parity.sh` and re-capture oracle baselines. Coordinate with P2.16 so the Phase 1 refactor and this rule land together |
| P2.16 | **Forecast-stack residual v1/v2 coupling — migrate live callers off legacy helpers** (doc 56, 20-Apr-26). BE CF and the v3 row builder still import directly from `cohort_forecast.py` (v1) and `cohort_forecast_v2.py` (v2) at runtime, and so does the engine itself (`forecast_state.py` → `build_upstream_carrier`). Doc 29e froze v2 as the parity oracle — the freeze is being violated every CF request. As a consequence v2 cannot be deleted. A concrete defect leaks through: `build_span_params` + `span_adapter` use κ=20 weak prior when no Bayesian posterior exists, ignoring the resolver's analytic α/β (D20 fallback), producing systematic undershoot on laggy edges (doc 50 truth-parity Δ ≈ 0.05–0.68). Must migrate before more engine consumers land. | [doc 56](56-forecast-stack-residual-v1-v2-coupling.md) — dependency map verified 20-Apr-26. Three v2 symbol uses reachable at runtime: `build_span_params` from CF handler (line 2317); `build_upstream_carrier` from v3 (`cohort_forecast_v3.py:466-467`); `build_upstream_carrier` from the engine (`forecast_state.py:312-313`). Plus the "frozen infrastructure" tier (v1 carrier helpers, span_kernel, span_evidence, span_adapter) is consumed by both CF and v3 | Phased plan in doc 56 §8: (0) red tests + oracle captures + dependency-audit script; (1) new `forecast_runtime.py` reading `ResolvedModelParams` + promoted span infrastructure; (2) cut engine over first; (3) cut v3 + CF handler; (4) delete `cohort_forecast_v2.py`, `span_adapter.py`, and (subject to dev-only overlap) `cohort_forecast.py`; (5) resume blocked consumer work on the clean boundary. Absorbs the v1/v2-retirement half of P2.10; the κ=20 defect is fixed structurally by Phase 1 |
...
| P3.25 | **Forecast function naming rename** (doc 56-naming, 20-Apr-26). `compute_forecast_summary` vs `compute_forecast_trajectory` have different input bindings, IS strategies, and return shapes, but identifiers don't distinguish them; the two return types collide on `completeness` with opposite semantics. Proposal: functions become `compute_forecast_summary` / `compute_forecast_trajectory`; return types become `ForecastSummary` / `ForecastTrajectory`; `completeness` splits into `completeness_conditioned` / `completeness_unconditioned`. `ForecastSummary` gains four fields doc 55 needs (`completeness_unconditioned` / `_sd`, `pp_rate_unconditioned` / `_sd`), populated from locals already computed. Hard-cut single-commit migration (no aliases). No behavioural change. | [doc 56-naming](56-forecast-function-naming.md). Scope: identifier substitution across ~31 files plus four field additions | Sequence before doc 52 / 55 / 47 implementation so new consumer code uses final identifiers from the start. P2.18 (surprise gauge rework) depends on the renamed return type |
...
6. **P2.16 — migrate CF / v3 / engine off v1/v2 helpers** (doc 56).
   Now a prerequisite for v1/v2 retirement. Pull together with
   **P2.19** (CF eligibility, doc 57 — replaces the abandoned κ=20
   band-aid) and **P3.25** (forecast function rename, doc 56-naming)
   — all three land on the same boundary and downstream consumers
   should see the final shape from the start. Blocks surprise-gauge
   follow-on (doc 55 / P2.18) and whole-graph forecast (doc 47).
...
16. **P3.23 — non-default a-anchoring for cohorts**. Design doc first.
17. **P3.24 — explicit drift modelling for frontier forecasting**
