...
6. **P2.16 — migrate CF / v3 / engine off v1/v2 helpers** (doc 56).
   Now a prerequisite for v1/v2 retirement. Fixes the κ=20 prior
   defect (laggy-edge undershoot from doc 50 truth-parity) as a
   structural side-effect of Phase 1. Blocks funnel hi/lo (P2.14),
   surprise-gauge follow-on (doc 55), and whole-graph forecast
   (doc 47).
...
13. **P2.14 — funnel hi/lo bars via BE CF machinery**. Depends on
    P2.2 (unified forecast-consumer contract) and P2.16 (CF/v3/engine
    cleanly on the runtime layer). Pull after both land.
