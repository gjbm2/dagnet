...
**Added 29-Apr — Bayes data lifecycle (production-readiness risk)**
- **Prove the model_vars / posterior lifecycle is stable end-to-end** —
  P3.27. Until this is proven, every fit is one auto-pull / reload /
  branch switch away from silent data loss. One source of silent loss
  has been fixed (auto-pull post-merge overwrite — see P3.27 detail);
  others are suspected but not yet exercised. Production-readiness gate
  for Bayes GA.
...
| P3.27 | **Bayes data lifecycle — model_vars / posterior persistence not proven stable end-to-end** (added 29-Apr-26). Stage-4(b) moved bayes data off the live graph and onto parameter files. The lifecycle around files-as-source-of-truth has multiple known gaps: (1) **auto-pull post-merge overwrite** — `pullFile` and `workspaceService.pullLatest` previously set `originalData = mergedData` and `isDirty = false` after a successful 3-way merge, so a second auto-pull saw `hasLocalChanges = false` and silently took the remote-wins branch, wiping the merged-in posteriors. **Fixed 29-Apr-26** in [repositoryOperationsService.ts:472-503](graph-editor/src/services/repositoryOperationsService.ts#L472) and [workspaceService.ts:1718-1745](graph-editor/src/services/workspaceService.ts#L1718) — `originalData` now tracks the parsed remote, `isDirty` becomes `(merged !== remote)`, and the workspace merge path no longer re-sets `isInitializing = true` (which would re-engage the [TabContext.updateFile L390](graph-editor/src/contexts/TabContext.tsx#L390) absorption path and silently fold local-only content into `originalData`). Regression tests added in `pullOperations.test.ts` and `workspaceService.integration.test.ts`. (2) **Post-fit live-edge stale-slice gap** — neither `applyPatch` nor `UpdateManager` knows the active DSL, so both project the bare `window()` aggregate slice onto `p.posterior` ([bayesPatchService.ts:307-380](graph-editor/src/services/bayesPatchService.ts#L307), [mappingConfigurations.ts:803-806](graph-editor/src/services/updateManager/mappingConfigurations.ts#L803-L806) hardcodes `projectProbabilityPosterior(value, '')`). The DSL-aware re-projection lives only in [useDSLReaggregation.ts:108-127](graph-editor/src/hooks/useDSLReaggregation.ts#L108-L127), gated by `lastContextedDSLRef.current === authoritativeDSL` — so after a fit lands, the gate trips (DSL unchanged), the effect skips, and the edge keeps the bare-aggregate projection until the user happens to change DSL. This is the gap created when `_posteriorSlices` was removed from the live graph (Stage 4(b)) AND Bayes started emitting context-qualified slices. **Fixed 29-Apr-26**: `applyPatchAndCascade` now dispatches `dagnet:bayesPosteriorsUpdated` after Tier 2 cascade succeeds ([bayesPatchService.ts:706-721](graph-editor/src/services/bayesPatchService.ts#L706-L721)); `useDSLReaggregation` listens, runs `contextLiveGraphForCurrentDsl(graph, paramResolver, currentDSL)` against the live graph and the active DSL, and `setGraph`s the result ([useDSLReaggregation.ts:128-163](graph-editor/src/hooks/useDSLReaggregation.ts#L128)). (3) **No atomic Bayes commit** — `applyPatch` marks N parameter files dirty plus the graph file dirty in one logical operation, but commits zero of them. The user must spot and commit N+1 files; if any are missed before the next reload / branch switch, the on-disk YAML rehydrates IDB without `posterior:`. (4) **Strict-clear in `posteriorSliceContexting.ts:73-85`** (73f) actively wipes `p.posterior` on every DSL change when the parameter file lacks `posterior.slices`. With (1) and (2) fixed this is a benign final guard, but it converts any *new* upstream gap into immediate visible "no bayes vars on edges" rather than a stale-but-present projection. (5) Other suspected paths not yet audited: `getOrCreateFile` rehydration after fresh workspace clone, `bayesPriorService.delete` callers, and any other writer that sets `isInitializing = true` near a merged parameter file | [repositoryOperationsService.ts:472-503](graph-editor/src/services/repositoryOperationsService.ts#L472), [workspaceService.ts:1718-1745](graph-editor/src/services/workspaceService.ts#L1718), [posteriorSliceContexting.ts:73-85](graph-editor/src/services/posteriorSliceContexting.ts#L73), [bayesPatchService.ts:241-512](graph-editor/src/services/bayesPatchService.ts#L241), [bayesPatchService.ts:706-721](graph-editor/src/services/bayesPatchService.ts#L706) (event dispatch), [useDSLReaggregation.ts:128-163](graph-editor/src/hooks/useDSLReaggregation.ts#L128) (event listener). Session log evidence: `BAYES_PATCH_APPLIED` 23-Apr-26 fit applied 4 edges in IDB; today (29-Apr-26) zero of 110 parameter YAMLs in the data repo carry a `posterior:` block, and the live `tmp.diag-state.json` shows no `posterior` / `slices` / `fit_history` keys anywhere. **Production-readiness blocker for Bayes GA**: until proven, every fit is one auto-pull / reload / branch switch away from silent data loss | Run the end-to-end acceptance sequence: (a) trigger fresh fit via DevBayesTrigger; (b) confirm `parameter-*.posterior.slices` lands in IDB and `isDirty: true`; (c) confirm `p.posterior` shows the active DSL's slice on canvas immediately after `BAYES_PATCH_APPLIED` (this is what the 29-Apr-26 (2) fix addresses — pre-fix the edge held the bare-aggregate slice until the user changed DSL); (d) trigger an auto-pull, then a second auto-pull — posterior must survive both (this is what (1) fix addresses); (e) change DSL on canvas, confirm `p.posterior` re-projects to the new slice; (f) reload page without committing — IDB rehydration must preserve posteriors. Fail at any step → identify and fix that link. Then add an "atomic Bayes commit" affordance after `applyPatchAndCascade` that stages all N+1 dirty files together. Soften the strict-clear into "clear only when we *had* a projection in this session" so any future upstream gap becomes visible (stale projection survives) rather than silent (everything blank) |
