...
### I-45: One resolution path; cases differ by degeneration, not branching

When the same logical answer is computed across multiple modes (window vs cohort, single-hop vs multi-hop, latency vs non-latency, scalar vs chart rows vs graph fields), there is one resolution chain that produces a single runtime object. Modes differ only by which sub-object degenerates: `window()` is `cohort()` with carrier-arrival = identity; non-latency is latency with lag = `δ(0)`; single-hop is multi-hop with a one-edge subject span. Parallel implementations diverge silently — fixes land in one branch and reappear in the other. If a new case can't be accommodated without an `if mode == ...` near the centre, the path is wrongly factored: fix the factoring, not the case. AP 58. Realisations: I-43 / I-44 (FE blend path runs latency and non-latency through one conjugate-blend), [COHORT_ANALYSIS_NUMERATOR_DENOMINATOR_SEMANTICS.md](COHORT_ANALYSIS_NUMERATOR_DENOMINATOR_SEMANTICS.md) "Implementation invariants" §1 (canonical statement for the BE cohort-forecast runtime; the trio with [FORECAST_RUNTIME_ARCHITECTURE.md](FORECAST_RUNTIME_ARCHITECTURE.md) and [FORECAST_STACK_DATA_FLOW.md](FORECAST_STACK_DATA_FLOW.md) is the engineering reference), `docs/current/project-bayes/73g-general-purpose-f14-problem-and-invariants.md` (forensic source); the selected-cohort projection cutover (`model_span_spine.project_selected_cohort_rows`: the BE cohort-forecast row reducer reads one conditioned/model operator and one empirical-evidence operator through a single DP core — `window()`, identity-carrier `cohort(A=X)`, and active `cohort(A≠X)` differ only by which operator family degenerates, not by branch; see `docs/archive/project-generalise/selected-cohort-projection-cutover-plan.md`).

### I-46: Projection layers don't re-decide semantics

Chart rows, scalar response fields (e.g. CF `p.mean`), graph-enrichment writes, fixture exports, and any other downstream consumer must read the already-resolved runtime object. They must not contain their own carrier, subject-span, population, or maturity logic — that re-derives semantics in the read layer, where the upstream object's choices can be silently reversed and the two answers diverge. If a projection needs information the upstream object doesn't expose, fix the upstream object; do not synthesise the missing piece in the projection. AP 58. See [COHORT_ANALYSIS_NUMERATOR_DENOMINATOR_SEMANTICS.md](COHORT_ANALYSIS_NUMERATOR_DENOMINATOR_SEMANTICS.md) "Implementation invariants" §9 for the canonical statement, [FORECAST_RUNTIME_ARCHITECTURE.md](FORECAST_RUNTIME_ARCHITECTURE.md) §7 for the runtime mechanics (`_project_runtime_rows` reads from runtime surfaces; it does not rebuild them), and `73g-general-purpose-f14-problem-and-invariants.md` for the worked example: a public BE scalar projecting raw under-matured `Σy / Σx` despite the resolved object claiming to be a maturity-aware `p∞`.
...
**This is actively policed.** Existing violations are debt to be retired gradually — they are **not precedent for new code**. New defensive patterns inside the CF engine will be reverted. Rules: [CF_ENGINE_DISCIPLINE.md](CF_ENGINE_DISCIPLINE.md). Tracker: [`cf-defensive-findings.md`](../project-generalise/cf-defensive-findings.md) (21 known violations). Stated principle: "no fallbacks within the engine — all defence, if any needed, should be at the perimeter."
