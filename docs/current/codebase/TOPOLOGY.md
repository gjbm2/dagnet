...
Three parallel analytic engines (`funnel_engine`, `daily_conversions_derivation`, `cohort_maturity_derivation`) remain as hold-outs pending unification onto a single mass-first reducer (audit F-1). The legacy trajectory engine `forecast_state.compute_forecast_trajectory` is post-73n legacy with two surviving callers (`surprise_gauge`, `daily_conversions` row annotation).
