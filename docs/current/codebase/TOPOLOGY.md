...
The Python topologically-sequenced MC pass that races FE topo. Per-edge IS conditioning on query-DSL-scoped snapshot evidence, implemented via the **primitive substrate** (five layers: contract types in `primitives.py` → evidence binding in `primitive_evidence.py` → single conditioning locus in `primitive_conditioning.py` → role-neutral span composition in `subject_span_composer.py` → role-labelled readout in `primitive_readout.py`). The substrate produces a `ResolvedCFRuntime` per request, consumed by the v3 row builder's selected-cohort mass reducer for E+F midpoint/fan, plus per-edge unconditioned overlays for F-mode and model-curve bands. Active `cohort(A != X)` mode uses **two** arrival-clock maps (carrier A-rooted, subject X-rooted with X-day root weights from the carrier's reach); window and `cohort(A = X)` are identity-carrier degeneracies of the same runtime object. CF supersedes FE blended `p.mean` and completeness when it lands.

Three parallel analytic engines (`funnel_engine`, `daily_conversions_derivation`, `cohort_maturity_derivation`) remain as hold-outs pending unification onto a single mass-first reducer (audit F-1). The legacy trajectory engine `forecast_state.compute_forecast_trajectory` has been deleted; `forecast_state.py` now holds only the `CohortEvidence` contract and the `_resolve_edge_p` / `_warn_legacy_pmean_carrier` helpers. Both former consumers now read the shared CF scalar bundle: `surprise_gauge` (`_compute_surprise_gauge`) via `prepare_cf_scalar_bundle` + `reduce_cf_scalars`, and `daily_conversions` via the date reducer over the same bundle.

Docs: [COHORT_ANALYSIS_NUMERATOR_DENOMINATOR_SEMANTICS.md](COHORT_ANALYSIS_NUMERATOR_DENOMINATOR_SEMANTICS.md) (semantics), [FORECAST_RUNTIME_ARCHITECTURE.md](FORECAST_RUNTIME_ARCHITECTURE.md) (runtime), [FORECAST_STACK_DATA_FLOW.md](FORECAST_STACK_DATA_FLOW.md) (I/O and persistence), [CF_PRIMITIVE_SUBSTRATE.md](CF_PRIMITIVE_SUBSTRATE.md) (substrate entry point), [CF_ROW_PIPELINE.md](CF_ROW_PIPELINE.md) (row engine, dual-prefix objects, selected-cohort reducer), [`cf-defensive-findings.md`](../project-generalise/cf-defensive-findings.md) (defensive-code tracker), [CF_HOLD_OUT_ENGINES.md](CF_HOLD_OUT_ENGINES.md) (parallel paths + legacy), [CF_RESIDUAL_GUARD.md](CF_RESIDUAL_GUARD.md), [DRAW_FAMILY_KEYING.md](DRAW_FAMILY_KEYING.md), [CF_REFACTOR_TRACKERS.md](CF_REFACTOR_TRACKERS.md), [STATS_SUBSYSTEMS.md](STATS_SUBSYSTEMS.md) §3.3, [BE_RUNNER_CLUSTER.md](BE_RUNNER_CLUSTER.md), [FE_BE_STATS_PARALLELISM.md](FE_BE_STATS_PARALLELISM.md).
...
- **`graph-editor/lib/api_handlers.py`** — 3,637 LOC; the BE dispatch monolith.
