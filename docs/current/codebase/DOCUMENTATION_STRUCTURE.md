...
1. **Current Docs** (`docs/current/`)
   ...
   - **Handover Notes** (`docs/current/handover/`) - Session continuity notes from previous agent sessions. If one exists for your current work area, **read it first** — contains decisions, rationale, and gotchas expensive to re-derive.
...
1. Check `docs/current/handover/` for handover notes relevant to the current work area
