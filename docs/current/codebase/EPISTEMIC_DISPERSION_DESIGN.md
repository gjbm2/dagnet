...
The runner code itself does not need behavioural change. Its existing `_pick_sd` candidate-ordering logic in [`model_resolver.py:613`](../../../graph-editor/lib/runner/model_resolver.py#L613) and its existing alpha_pred-first proposal preference in [`primitive_conditioning.py`](../../../graph-editor/lib/runner/primitive_conditioning.py) are now correct under the broader design — the analytic source emits a real predictive `α_pred` and the runner consumes it without further inflation.
...
**Runner consumer.** The back-end runner requires no behavioural change. The dispersion-field `_pick_sd` candidate-ordering logic in [`model_resolver.py`](../../../graph-editor/lib/runner/model_resolver.py) and the `alpha_pred`-first IS-proposal preference in [`primitive_conditioning.py`](../../../graph-editor/lib/runner/primitive_conditioning.py) are correct under the new design — once the analytic source emits real predictive fields, those fields are consumed without further inflation. A clarifying comment at the `_pick_sd` site documents the now-correct semantics.
