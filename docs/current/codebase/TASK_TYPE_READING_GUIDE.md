...
- **CF orientation** **(full, FIRST, if touching `cohort_forecast_v3` or any conditioned-forecast surface)** — one-page map of the CF cluster: file → role lookup, end-to-end stage flow, canonical reading order across the deeper docs.
  - **CF_MAP.md** — read first; tells you which of the below docs you actually need for your task.
- **CF / forecast machinery core** **(full, in this order, after CF_MAP)** — these four together are the canonical engineering reference for the runtime; the "Implementation invariants" section in #1 is binding:
  ...
  2. **FORECAST_PREPARATION.md** — the BE preparation layer between dispatch and runtime: subject resolution, snapshot DB fetch + regime selection, frame composition, request envelope plan. Read before opening `forecast_preparation.py`, `analysis_subject_resolution.py`, or `request_envelope.py`.
  3. **FORECAST_RUNTIME_ARCHITECTURE.md** — what happens *inside* the CF kernel: `ResolvedCFRuntime`, primitive conditioning and composition, identity-carrier degeneracy, selected-Cohort reduction, selected A-clock evidence, row projection, public scalars and provenance
  4. **FORECAST_STACK_DATA_FLOW.md** — labelled-interface (I1–I17) data flow and persistence boundaries around the runtime; CF response → graph apply mapping
- **CF substrate detail docs** **(full if working inside `lib/runner/primitive_*.py`, `subject_span_composer.py`, `prefix_arrival.py`, or the row pipeline in `cohort_forecast_v3.py`)** — these resolve the `§A.*`-style references that pepper the CF code:
  ...
  7. **CF_REFACTOR_TRACKERS.md** **(ref)** — index of the in-flight design-doc trackers under `docs/current/` (`cohort-1apr-falling-k-problem-statement.md`, `cohort-maturity-evidence-coverage-design.md`, `selected-a-clock-retrieval-frontier-provenance-proposal.md`, `snapshot-fetch-envelope-design.md`, `cohort-outside-in-post-73n-regression-tracker.md`, etc.). Consult when a code comment cites a `§A.4`-style reference.
...
- `CF_REFACTOR_TRACKERS.md` — index of in-flight CF design-doc trackers under `docs/current/`
