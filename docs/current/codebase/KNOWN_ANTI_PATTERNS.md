...
**Open inventory not yet here**: the CF defensive-coding audit ([`docs/current/cf-defensive-coding-audit.md`](../cf-defensive-coding-audit.md), 12-May-26) documents 21 findings (7 HIGH, 9 MEDIUM, 5 LOW) across the CF runtime — `or 0.0` cascades, `np.clip` boundary corruption, `try/except: pass` swallow, identity-carrier branching explosion, and five parallel `ΣY / ΣX` code paths. These are not yet entries here because they require substantial multi-file remediation and are blocked on architectural decisions (H-5 identity-carrier unification, H-2 funnel zero-substitution, F-1 mass-first reducer unification). As each finding is remediated, the corresponding cross-cutting pattern should migrate here so future agents recognise the trap. Executive summary: [CF_DEFENSIVE_FINDINGS.md](CF_DEFENSIVE_FINDINGS.md). The shared structural pattern most of them embody is already captured here as AP58 ("forking by case instead of degenerating one path") and is codified as [INVARIANTS.md](INVARIANTS.md) I-47 / I-48.
...
**Outstanding instance** (1-May-26): `build_cohort_evidence_from_frames` in `graph-editor/lib/runner/cohort_forecast_v3.py` contains a second-variant AP58 fork on the count axis. An `is_window`-gated population fallback at `:750-769` runs in parallel with a specialised carrier-projection rebuild at `:775-803`. The two forks produce systematically different `obs_x`/`obs_y` per τ between is_window=True and is_window=False; the trajectory engine derives different rate_draws as a consequence. Surfaced by 73m Stage 5's router unification (the legacy router fork was masking the divergence by sending non-latency targets through the closed-form path, never touching the evidence-builder fork). 73n owns the fix via its request-scoped primitive registry + composition pass + projection pass — projection becomes a readout of composed primitives instead of rebuilding evidence locally. Four strict-xfail tests in `test_cohort_factorised_outside_in.py` carry precise `reason=` lines naming the fork and 73n's primitive registry as the flip-to-green target; `strict=True` ensures any XPASS during 73n surfaces as a suite failure prompting marker removal. Full record in [`docs/current/project-bayes/73m-stage-0-baseline.md`](../project-bayes/73m-stage-0-baseline.md) §§9-12.
