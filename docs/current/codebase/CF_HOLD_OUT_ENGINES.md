...
**Do not add a fifth parallel `ΣY / ΣX` reducer.** If a new analytic needs cumulative-Y-over-cumulative-X, wire it into the canonical mass-first projection (`build_cf_projection_bundle` / `selected_projection.ef_rate_draws`) or, if that doesn't yet expose what you need, **fix the substrate**, don't fork.
...
|---|---|---|---|---|
| **Selected-cohort projection** | `cohort_forecast_v3.build_cf_projection_bundle` (`selected_projection.ef_rate_draws`) | `ResolvedCFRuntime` (composed primitives), `engine_cohorts`, projection bases | `CFProjectionBundle`; per-particle `ef_rate_draws (S, T)`; the E+F authority | **Canonical**, primitive-substrate-backed |
...
The mass-first E+F authority is `build_cf_projection_bundle` (producing `selected_projection.ef_rate_draws`). It sums **mass** across selected cohorts first (per-particle `ΣX_total(s, τ)` and `ΣY_total(s, τ)`) and divides **once** at the end, with `NaN` cells where the denominator is zero. The denominator pool is `x_frozen` (observed prefix) + `(a_pop − x_frozen) × R_x` (Pop C future arrivals via conditional carrier residual). The numerator pool is `y_frozen` + Pop D residual (`(x_frozen − y_frozen) × R_subject_calibrated`) + Pop C convolution (`(a_pop − x_frozen) × Conv_PopC`). Identity carrier degenerates: no Pop C, denominator stays at `x_frozen`. See [CF_ROW_PIPELINE.md](CF_ROW_PIPELINE.md) §4 for the full algebra. The reducer reads composed primitives directly from `ResolvedCFRuntime.composed_subject` / `composed_carrier`; it does not re-condition.
...
`daily_conversions_derivation.py` (170 LOC). Computes per-day conversion attribution and per-cohort rate from snapshot rows. Two outputs:
...
## The retired trajectory engine

The legacy trajectory engine (`compute_forecast_trajectory` and its support cast) was **deleted in 73q Phase 7**. `forecast_state.py` is now a 133-LOC stub retaining only `_warn_legacy_pmean_carrier`, `_resolve_edge_p` (the documented pre-Stage-4 legacy `p.mean` carrier fallback, doc 73b §6.5), and the `CohortEvidence` dataclass consumed by the v3 conditioned-forecast runtime.

The trajectory engine was the pre-substrate cohort-loop projector: it built per-particle `(p, μ, σ, onset)` draws from `ResolvedModelParams`, ran the v2-style cohort loop with Pop C / Pop D mass mixing, and returned `ForecastTrajectory` with `rate_draws`, `model_rate_draws`, `completeness_mean`, `completeness_unconditioned`, `pp_rate_unconditioned`.

Both surfaces that used to depend on it have migrated to the shared CF projection bundle:

- **Surprise gauge** (doc 55): `_compute_surprise_gauge` (api_handlers.py) now reads the CF scalar bundle via `prepare_cf_scalar_bundle` + `reduce_cf_scalars`.
- **Daily-conversions v3 chart**: bundle-only, via the date reducer (`reduce_daily_conversions_rows`) over the shared `CFProjectionBundle`.

The trajectory engine and its support cast (`ForecastTrajectory`, `compute_completeness_with_sd`, `NodeArrivalState`, `build_node_arrival_cache`) are gone. The `XProvider` / `build_x_provider_from_graph` carrier plumbing still exists but lives in `forecast_runtime.py`, not here.
...
1. **First check whether the substrate already produces what you need.** `ResolvedCFRuntime.composed_subject.span_p_draws` + `composed_carrier.cdf_draws` are the primitive-backed model surfaces. `build_cf_projection_bundle` / `selected_projection.ef_rate_draws` is the mass-first projection. If your analysis can read from these, do not start a new engine — wire into the substrate.
