...
declaring the same dim id share the same rendered file. Bootstrapping one
truth can change the file another truth depends on. Without governance
this produces silent data corruption — DB rows registered under one
dim-definition hash outlive the file that produced them.
...
1. **Identical definitions**: many truths declare the dim with the same
   ...
   the file. Common case within a family (e.g. all `synth-*-sparse-*`
   graphs share `synth-channel` for comparable regime structure).
2. **Deliberate redefinition**: a newer truth changes the dim (new
   labels, new sources). The file overwrite is intentional; every graph
   previously bootstrapped against the old definition is now stale and
   must re-bootstrap before being trusted.
...
- A truth silently overwrites a sibling's file and nobody notices,
  ...
  correspond to the on-disk dim. Exactly the defect that motivated
  this contract.
...
2. **Meta sidecar pin** (`save_synth_meta`). Every bootstrap records
   ...
   `synth-meta.json`. The anchor the freshness check compares against.
...
on that dimension — values mean the same thing, sources are the same,
and a joint analysis across those truths would be meaningful.

Namespace the dim id (e.g. `synth-channel-lifecycle` instead of
`synth-channel`) when semantics genuinely differ — different labels,
different weights, different temporal lifecycles. Sharing a dim id
across semantically distinct definitions is a modelling error that
