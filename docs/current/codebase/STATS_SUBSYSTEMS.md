...
**Confusion 5: "the funnel uses compute_forecast_trajectory"**
...
**Confusion 8: "`model_vars[analytic_be].alpha, beta` can be used as a prior"**
It can't — it's already a posterior, and a query-scoped one. Both FE
topo and BE topo derive `α, β` from **query-scoped** `total_k,
total_n` as a Jeffreys posterior (`α = k+1, β = n-k+1`). Using it as
a prior for a conjugate Beta-Binomial update with query-scoped
evidence would count the same evidence twice.
...
| `analytic_be` | **Query-scoped** (Jeffreys posterior from DSL-windowed `total_k, total_n`) | **Unscoped** (full history to avoid survivor bias) |
| `analytic` | **Query-scoped** (same Jeffreys construction, FE-computed) | **Unscoped** (same reasoning) |
...
query-scoped cohort evidence. When `analytic` or `analytic_be` is
promoted, its `α, β` is already the query-scoped answer — consumers
should read it directly, not update it further. See
...
**Rule of thumb**: if you're writing an **analysis runner** or a **chart builder**, the two public entry points you need are `handle_conditioned_forecast` (for CF-conditioned scalars) and `handle_stats_topo_pass` (for analytic scalars). Everything else is an inner kernel. If you find yourself importing from `forecast_state.py` or `span_kernel.py` inside a runner, stop and re-read this table.
