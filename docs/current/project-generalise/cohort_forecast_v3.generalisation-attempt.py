...
class _SelectedRoleSupport:
    ...
    carrier_endpoint_tau: Optional[int] = None
...
class _SelectedSourceDayMass:
    """Source-layer selected timing mass.

    This object is not the evidence-local downstream value ledger. The selected
    evidence value path consumes only the admitted seed at the denominator node
    from this surface; downstream selected mass is then produced by the generic
    push-forward evaluator through observed-rate transition operators.
    """
...
@dataclass(frozen=True)
class _SelectedEvidenceSeedOperator:
    """Admitted B_{R->X} seed operator for selected evidence propagation.

    This is the value-path seed consumed by the generic push-forward
    evaluator. It may be identity (`window()` / `cohort(A=X)`) or active
    carrier supplied, but the evaluator never needs to know which.
    """

    node_id: str
    mass_by_anchor_source_day: Mapping[str, Mapping[str, float]]
    provenance: Mapping[str, Any] = field(default_factory=dict)


@dataclass(frozen=True)
class _SelectedEvidenceTransitionOperator:
    """Admitted K_i transition operator for selected evidence propagation."""

    edge_id: str
    from_node: str
    to_node: str
    rate_surface: str
    cumulative_age_rates: Tuple[float, ...] = ()
    cumulative_source_day_rates: Mapping[
        str, Mapping[str, Tuple[float, ...]]
    ] = field(default_factory=dict)
    timing_family: Optional[str] = None
    operator_status: str = 'unspecified'
    provenance: Mapping[str, Any] = field(default_factory=dict)


@dataclass(frozen=True)
class _SelectedEvidencePushForwardResult:
    """Generic selected-mass push-forward result."""

    ledger: _SelectedEvidencePropagationLedger
    terminal_cumulative_by_anchor: Mapping[str, Mapping[int, float]]
...
class ResolvedCFRuntime:
    ...
    # (source_node, destination_node). Used to construct the admitted
    # denominator seed for selected evidence. Downstream evidence-local
    # mass is produced by the selected-evidence push-forward evaluator,
    # not by reading this source-layer timing surface.
...
def _build_selected_source_day_mass(
    *,
    ...
    The selected evidence value path treats this as seed material only where
    the operator-supply boundary admits it as `B_{R->X}`. It must not be read as
    downstream evidence-local mass for later subject primitives; the generic
    selected-evidence push-forward evaluator owns that downstream mass.

    For each U in the set of subject primitive source nodes, this source-layer
    diagnostic surface composes A → U via the shared timing-span composer over
    the request's union of carrier and subject source-layer transitions.
    Difference the resulting density CDF into a reach-preserving PMF and
    convolve with N_cohort per anchor:
    ...
    maps — no second timing implementation and no chaining of
    post-conditioning posteriors.
    """
...
def _build_source_day_rate_cache(
    ...
    *,
    max_tau: int,
) -> Mapping[str, Mapping[str, Mapping[str, Tuple[Optional[float], ...]]]]:
    ...
    performs the forward-fill once per (edge, selected anchor, source_day).
    It deliberately uses the primitive-bound, anchor-placed buckets rather
    than the raw local-window bucket surface so active cohort operators consume
    selected-root-attributed rows.
    """
    horizon = max(int(max_tau), 0) + 1
    cache: Dict[str, Dict[str, Dict[str, Tuple[Optional[float], ...]]]] = {}
    for edge_id, by_anchor in buckets.edge_nk_by_source_day.items():
        edge_cache: Dict[str, Dict[str, Tuple[Optional[float], ...]]] = {}
        for anchor_day, by_source_day in by_anchor.items():
            anchor_cache: Dict[str, Tuple[Optional[float], ...]] = {}
            for source_day, nk_by_tau in by_source_day.items():
                source_offset = _days_between(anchor_day, source_day)
                if source_offset is None:
                    continue
                series: List[Optional[float]] = []
                latest_rate: Optional[float] = None
                nk_by_age: Dict[int, Tuple[float, float]] = {}
                for tau, nk in nk_by_tau.items():
                    local_age = int(tau) - int(source_offset)
                    if 0 <= local_age < horizon:
                        nk_by_age[local_age] = nk
                for age in range(horizon):
                    if int(age) in nk_by_age:
                        n_val, k_val = nk_by_age[int(age)]
                        if n_val > 0.0:
                            latest_rate = max(
                                0.0,
                                min(1.0, float(k_val) / float(n_val)),
                            )
                    series.append(latest_rate)
                anchor_cache[str(source_day)[:10]] = tuple(series)
            if anchor_cache:
                edge_cache[str(anchor_day)[:10]] = anchor_cache
        if edge_cache:
...
def _latest_defined_rate(rates: Sequence[Optional[float]]) -> Optional[float]:
    latest: Optional[float] = None
    for value in rates:
        if value is not None:
            latest = max(0.0, min(1.0, float(value)))
    return latest


def _rate_series_requires_monotone_repair(
    rates: Sequence[Optional[float]],
) -> bool:
    ...
    for value in rates:
        ...
        if running_max is not None and current + 1e-12 < running_max:
            return True
        running_max = current if running_max is None else max(running_max, current)
    return False


def _primitive_uses_delta0_kernel(
    primitive: Optional[ConditionedTransitionPrimitive],
) -> bool:
    def _family_value(family: Any) -> Optional[str]:
        return getattr(family, 'value', family)

    if primitive is None:
        return False
    if _family_value(getattr(primitive, 'timing_family', None)) == TimingFamily.NON_LATENT.value:
        return True
    timing = (
        getattr(primitive, 'timing_posterior', None)
        or getattr(primitive, 'timing_prior', None)
    )
    if timing is None:
        return False
    if _family_value(getattr(timing, 'family', None)) == TimingFamily.NON_LATENT.value:
        return True
    if (
        _family_value(getattr(timing, 'family', None)) == TimingFamily.DETERMINISTIC.value
        and int(getattr(timing, 'deterministic_shift_days', 0) or 0) == 0
    ):
        return True
    return False
...
def _build_selected_evidence_seed_operator(
    *,
    selected_source_day_mass: _SelectedSourceDayMass,
    denominator_node: str,
    anchor_days: Sequence[str],
    seed_operator_source: str,
) -> Optional[_SelectedEvidenceSeedOperator]:
    anchor_keys = tuple(str(d)[:10] for d in anchor_days)
    seed_by_anchor: Dict[str, Dict[str, float]] = {}
    endpoint_cdf = tuple(
        float(v)
        for v in selected_source_day_mass.endpoint_cdf_by_node.get(
    ...
    for anchor in anchor_keys:
        n_cohort = float(
            selected_source_day_mass.n_cohort_by_anchor.get(anchor, 0.0)
            or 0.0
        )
        per_day: Dict[str, float] = {}
        if endpoint_cdf and n_cohort > 0.0:
            try:
            ...
            if anchor_d is not None:
                for tau, raw_cdf in enumerate(endpoint_cdf):
                    cdf_val = max(prev, max(0.0, min(1.0, float(raw_cdf))))
                    inc = max(0.0, cdf_val - prev)
                    prev = cdf_val
                    if inc <= 0.0:
                        continue
                    source_day = (anchor_d + _timedelta(days=int(tau))).isoformat()
                    per_day[source_day] = per_day.get(source_day, 0.0) + n_cohort * inc
        if not per_day:
            source_day_mass = (
            ...
            if source_day_mass:
                per_day = dict(source_day_mass)
        if per_day:
            seed_by_anchor[anchor] = per_day
    if not seed_by_anchor:
        return None
    return _SelectedEvidenceSeedOperator(
        node_id=str(denominator_node),
        mass_by_anchor_source_day=seed_by_anchor,
        provenance={
            'source': seed_operator_source,
            'discretisation': (
                'endpoint_cdf_increments_match_x_prefix'
                if endpoint_cdf else 'source_day_mass_fallback'
            ),
            'denominator_node': str(denominator_node),
            'anchor_count': len(seed_by_anchor),
        },
    )


def _build_selected_evidence_transition_operators(
    *,
    buckets: Optional['_SubjectChainEvidenceBuckets'],
    max_tau: int,
    denominator_node: str,
    primitive_by_edge_id: Optional[Mapping[str, ConditionedTransitionPrimitive]] = None,
    downstream_rate_policy: str = 'age_only',
) -> Optional[Tuple[_SelectedEvidenceTransitionOperator, ...]]:
    """Supply admitted K_i operators; policy lives here, not in evaluator."""
    if buckets is None:
        return None
    ...
    primitives_by_edge = dict(primitive_by_edge_id or {})
    operators: List[_SelectedEvidenceTransitionOperator] = []
    ...
    for from_id, to_id, edge_id in topology_edges:
        ...
        rate_surface = (
            'source_day_specific'
            if (
                from_id_str == str(denominator_node)
                or downstream_rate_policy == 'source_day_specific'
            )
            else 'age_only'
        )
        primitive = primitives_by_edge.get(str(edge_id))
        is_delta0 = _primitive_uses_delta0_kernel(primitive)
        timing_family_raw = getattr(primitive, 'timing_family', None)
        timing_family = getattr(timing_family_raw, 'value', timing_family_raw)

        if rate_surface == 'age_only':
            raw_rates = tuple(age_only_rate_cache.get(str(edge_id), ()))
            repaired = _rate_series_requires_monotone_repair(raw_rates)
            if is_delta0:
                total_rate = _latest_defined_rate(raw_rates)
                edge_rates = (
                    np.full(horizon, float(total_rate), dtype=np.float64)
                    if total_rate is not None
                    else np.zeros(horizon, dtype=np.float64)
                )
            else:
            ...
            if not any(value is not None for value in raw_rates):
                operators.append(_SelectedEvidenceTransitionOperator(
                    edge_id=str(edge_id),
                    ...
                    provenance={
                        'reason': 'no_positive_admitted_age_only_rate',
                    },
                ))
                continue
            status = (
                ...
                if is_delta0
                    ...
                    'precomputed_age_only_monotone_repaired'
                    if repaired else 'precomputed_age_only'
                )
            )
            operators.append(_SelectedEvidenceTransitionOperator(
                edge_id=str(edge_id),
                ...
                to_node=to_id_str,
                rate_surface=rate_surface,
                cumulative_age_rates=tuple(float(v) for v in edge_rates),
                timing_family=timing_family,
                operator_status=status,
            ))
            continue

        source_rates_by_anchor: Dict[str, Dict[str, Tuple[float, ...]]] = {}
        edge_repaired = False
        for anchor_day, by_source_day in (
            source_day_rate_cache.get(str(edge_id), {}).items()
        ):
            anchor_rates: Dict[str, Tuple[float, ...]] = {}
            for source_day, raw_rates in by_source_day.items():
                raw_rates_tuple = tuple(raw_rates)
                if not any(value is not None for value in raw_rates_tuple):
                    continue
                if _rate_series_requires_monotone_repair(raw_rates_tuple):
                    edge_repaired = True
                if is_delta0:
                    total_rate = _latest_defined_rate(raw_rates_tuple)
                    if total_rate is None:
                        continue
                    rates = np.full(horizon, float(total_rate), dtype=np.float64)
                else:
                    rates = _monotone_rate_array(raw_rates_tuple, horizon=horizon)
                anchor_rates[str(source_day)[:10]] = tuple(
                    float(v) for v in rates
                )
            if anchor_rates:
                source_rates_by_anchor[str(anchor_day)[:10]] = anchor_rates
        if not source_rates_by_anchor:
            operators.append(_SelectedEvidenceTransitionOperator(
                edge_id=str(edge_id),
                from_node=from_id_str,
                to_node=to_id_str,
                rate_surface=rate_surface,
                timing_family=timing_family,
                operator_status='missing_source_day_specific_rate',
                provenance={
                    'reason': 'no_positive_admitted_source_day_specific_rate',
                },
            ))
            continue
        status = (
            'delta0_source_day_specific'
            if is_delta0
            else (
                'source_day_specific_monotone_repaired'
                if edge_repaired else 'source_day_specific'
            )
        )
        operators.append(_SelectedEvidenceTransitionOperator(
            edge_id=str(edge_id),
            ...
            to_node=to_id_str,
            rate_surface=rate_surface,
            cumulative_source_day_rates=source_rates_by_anchor,
            timing_family=timing_family,
            operator_status=status,
        ))

    return tuple(operators) if operators else None


def _evaluate_selected_evidence_push_forward(
    *,
    seed_operator: _SelectedEvidenceSeedOperator,
    transition_operators: Sequence[_SelectedEvidenceTransitionOperator],
    anchor_days: Sequence[str],
    max_tau: int,
    end_node: str,
    emit_diagnostics: bool = False,
) -> Optional[_SelectedEvidencePushForwardResult]:
    """Generic deterministic selected-mass push-forward.

    The evaluator composes already-admitted operators. It has no knowledge of
    query mode, carrier/subject roles, path length, or timing policy.
    """
    horizon = int(max_tau) + 1
    by_node: Dict[str, Dict[str, Dict[str, float]]] = {
        seed_operator.node_id: {
            str(anchor)[:10]: dict(
                seed_operator.mass_by_anchor_source_day.get(str(anchor)[:10], {}),
            )
    ...
    missing_required_operator = False
    missing_required_samples: List[Mapping[str, Any]] = []

    for operator in transition_operators:
        source_mass_by_anchor = by_node.get(operator.from_node, {})
        if not source_mass_by_anchor:
            edge_provenance.append({
                'edge_id': operator.edge_id,
                'from_node': operator.from_node,
                'to_node': operator.to_node,
                'mass_source': 'absent_evidence_local_mass_for_node',
                'rate_surface': operator.rate_surface,
                'operator_status': operator.operator_status,
                'anchors_with_cumulative': [],
            })
            continue

        terminal_edge = str(operator.to_node) == str(end_node)
        ...
        if operator.rate_surface == 'age_only':
            edge_rates = np.asarray(operator.cumulative_age_rates, dtype=np.float64)
            if edge_rates.size == 0 or not np.any(edge_rates > 0.0):
                edge_provenance.append({
                    'edge_id': operator.edge_id,
                    'from_node': operator.from_node,
                    'to_node': operator.to_node,
                ...
                continue
            if edge_rates.size < horizon:
                edge_rates = np.pad(
                    edge_rates,
                    (0, horizon - edge_rates.size),
                    mode='edge',
                )
            else:
                edge_rates = edge_rates[:horizon]
            ...
            for anchor_day, mass_by_source_day in source_mass_by_anchor.items():
                if not mass_by_source_day:
                    continue
                anchor_cum = {tau: 0.0 for tau in range(horizon)}
                ...
                for source_day, source_mass in mass_by_source_day.items():
                    ...
                    if offset is None or offset > int(max_tau):
                        continue
                    source_rates = (
                        operator.cumulative_source_day_rates
                        .get(str(anchor_day)[:10], {})
                        .get(str(source_day)[:10], ())
                    )
                    if not source_rates:
                        missing_required_operator = True
                        if len(missing_required_samples) < 20:
                            missing_required_samples.append({
                                'edge_id': operator.edge_id,
                                'anchor_day': str(anchor_day)[:10],
                                'source_day': str(source_day)[:10],
                        ...
                        edge_provenance.append({
                            ...
                            'mass_source': 'selected_evidence_propagation_ledger',
                            'rate_surface': operator.rate_surface,
                            'operator_status': 'missing_required_source_day_rate',
                            'anchor_day': str(anchor_day)[:10],
                            'source_day': str(source_day)[:10],
                        })
                        continue
                    ...
                    prev_rate = 0.0
                    for age in range(start_age, horizon):
                        tau = int(offset) + int(age)
                        if tau < 0 or tau >= horizon:
                            continue
                        rate = (
                            float(source_rates[int(age)])
                            if int(age) < len(source_rates)
                            else float(source_rates[-1])
                        )
                        rate = max(prev_rate, max(0.0, min(1.0, rate)))
                        if terminal_edge:
        ...
        if terminal_edge:
            ...
            if emit_diagnostics and diagnostic_by_anchor:
                dual_eval_by_edge[operator.edge_id] = diagnostic_by_anchor
        else:
            existing = by_node.setdefault(operator.to_node, {})
        ...
        edge_provenance.append({
            ...
            'mass_source': 'selected_evidence_propagation_ledger',
            'rate_surface': operator.rate_surface,
            'timing_family': operator.timing_family,
            'operator_status': operator.operator_status,
            'anchors_with_cumulative': sorted(cumulative_by_anchor.keys()),
        })

    diagnostics = {
        'missing_required_operator': bool(missing_required_operator),
        'missing_required_samples': tuple(missing_required_samples),
        'terminal_anchor_count': len(terminal_anchor_cum),
    }
    if not terminal_anchor_cum:
        if emit_diagnostics:
            return _SelectedEvidencePushForwardResult(
                ledger=_SelectedEvidencePropagationLedger(
                    by_node={
                        node: {
...
def _build_evidence_local_rate_attributed_subject_prefix(
    *,
    buckets: Optional['_SubjectChainEvidenceBuckets'],
    selected_source_day_mass: _SelectedSourceDayMass,
    seed_operator: Optional[_SelectedEvidenceSeedOperator] = None,
    anchor_days: Sequence[str],
    max_tau: int,
    denominator_node: str,
    end_node: str,
    primitive_by_edge_id: Optional[Mapping[str, ConditionedTransitionPrimitive]] = None,
    downstream_rate_policy: str = 'age_only',
    seed_operator_source: str = 'unspecified',
    emit_diagnostics: bool = False,
) -> Optional[_RateAttributedSubjectPrefix]:
    """Build terminal selected evidence via generic operator push-forward."""
    admitted_seed_operator = seed_operator or _build_selected_evidence_seed_operator(
        selected_source_day_mass=selected_source_day_mass,
        denominator_node=denominator_node,
        anchor_days=anchor_days,
        seed_operator_source=seed_operator_source,
    )
    transition_operators = _build_selected_evidence_transition_operators(
        buckets=buckets,
        max_tau=max_tau,
        denominator_node=denominator_node,
        primitive_by_edge_id=primitive_by_edge_id,
        downstream_rate_policy=downstream_rate_policy,
    )
    if admitted_seed_operator is None or not transition_operators:
        return None

    result = _evaluate_selected_evidence_push_forward(
        seed_operator=admitted_seed_operator,
    ...
    if result is None:
        return None
    if not result.terminal_cumulative_by_anchor:
        if emit_diagnostics:
            return _RateAttributedSubjectPrefix(
                cumulative_by_anchor={},
                landing_coverage_by_anchor={},
                edge_provenance=result.ledger.edge_provenance,
                aggregate_provenance={
                    'composition': 'rate_attributed_local_evidence_propagation.v1',
                    'evaluator_scope': 'generic_selected_evidence_push_forward',
                    'denominator_node': str(denominator_node),
                    'end_node': str(end_node),
                    'edge_count': len(transition_operators),
                    'ledger_node_count': len(result.ledger.by_node),
                    'coverage_source': 'observed_span_surface_only',
                    'seed_operator': dict(admitted_seed_operator.provenance),
                    'downstream_rate_policy': downstream_rate_policy,
    ...
    return _RateAttributedSubjectPrefix(
        cumulative_by_anchor=dict(result.terminal_cumulative_by_anchor),
        # Support/coverage is deliberately NOT derived from this value ledger.
        # Public coverage comes from ObservedSpanEvidenceSurface exact-row
        # placements; deriving support from rate movement would conflate value
        # propagation with observation support.
        landing_coverage_by_anchor={},
        edge_provenance=result.ledger.edge_provenance,
        aggregate_provenance={
            'composition': 'rate_attributed_local_evidence_propagation.v1',
        ...
        diagnostic_dual_eval_by_edge=dict(result.diagnostic_dual_eval_by_edge),
    )
...
def _build_rate_attributed_subject_prefix(
    *,
    ...
    """Thin compatibility adapter over the generic selected-evidence path."""
...
def _build_selected_a_clock_evidence_from_runtime(
    ...
    *,
    ...
    Cell amplitudes come from `runtime.selected_x_prefix` for X and from a
    rate-attributed Y prefix produced by the generic selected-evidence
    push-forward evaluator. `_SelectedSourceDayMass` may supply the admitted
    denominator seed; downstream subject-source mass is produced only by
    observed-rate transition operators.
    ...
    y_prefix = _build_rate_attributed_subject_prefix(
        ...
        downstream_rate_policy=(
            'age_only' if is_identity_carrier else 'source_day_specific'
        ),
        seed_operator_source=(
            'identity_dirac_at_denominator'
            if is_identity_carrier
            else 'resolved_carrier_seed_at_denominator'
        ),
        emit_diagnostics=emit_diagnostics,
    )
    if y_prefix is None:
        _record_diag(
            'missing_admitted_subject_transition_operator',
    ...
    for anchor_day in anchor_days:
        ...
        # Cell-emit τ set: observation markers only. carrier_surface and
        # subject_surface are the observation-marker sources (each τ here
        # corresponds to a real row landing). y_prefix is a value-bearing
        # CDF object, not a marker source — its τ set comes from the
        # convolution and is dense over [first_non_zero, horizon], which
        # would extend the cell-emit set past the observation frontier and
        # let composer-extrapolated x_prefix amplitude bleed into evidence_x.
        ...
        for tau in sorted(tau_values):
            ...
            # X_prefix and Y_prefix share one admitted denominator seed.
            # Downstream Y mass is produced by the generic push-forward
            # through sub-probability K_i operators, so Y should remain
            # bounded by the selected X seed without projection-time repair.
            ...
            # forward when the value is flat. Coverage remains role-specific:
            # a fresh subject row cannot make X coverage fresh, and a fresh
            # carrier row cannot make Y coverage fresh. Row-level coverage is
            # the weaker side when aggregate_by_tau projects these fields.
            ...
            carrier_landing_coverage = carrier_exact_coverage
            subject_landing_coverage = subject_exact_coverage
    ...
    #
    # Active carrier (A != X): paired support is the overlap of endpoint
    # support for the composed carrier and the composed subject. This must
    # not be the min of raw primitive freshness: a short upstream carrier
    # primitive may be fully saturated while downstream carrier evidence keeps
    # extending A→X endpoint support. It also must not be the display value
    # frontier, because forward-filled values are not fresh observations.
    #
    # Identity carrier (window or A == X): the carrier is structural identity
    # with no retrieval-bearing role, so paired_tau = subject_tau when present.
    ...
    for anchor_day in anchor_days:
        if carrier_surface is not None:
            ...
            carrier_endpoint_taus = [
                int(tau)
                for tau in carrier_surface.tau_values_for_anchor(anchor_day)
            ]
        else:
            carrier_taus = []
            carrier_endpoint_taus = []
        if subject_surface is not None:
            ...
            subject_endpoint_taus = [
                int(tau)
                for tau in subject_surface.tau_values_for_anchor(anchor_day)
            ]
        else:
            subject_fresh_taus = []
            subject_endpoint_taus = []
        carrier_fresh_tau = max(carrier_taus) if carrier_taus else None
        carrier_endpoint_tau = (
            max(carrier_endpoint_taus) if carrier_endpoint_taus else None
        )
        ...
        subject_tau = (
            max(subject_endpoint_taus) if subject_endpoint_taus else None
        )
        if is_identity_carrier:
            paired_tau = subject_tau
        else:
            paired_tau = (
                min(carrier_endpoint_tau, subject_tau)
                if carrier_endpoint_tau is not None and subject_tau is not None
                else None
            )
        if (
            carrier_fresh_tau is None
            and carrier_endpoint_tau is None
        ...
        strict_support[anchor_day] = _SelectedRoleSupport(
            carrier_tau=carrier_endpoint_tau,
            carrier_fresh_tau=carrier_fresh_tau,
            carrier_endpoint_tau=carrier_endpoint_tau,
        ...
        runtime.projection_provenance['selected_a_clock_evidence'] = {
            ...
            'strict_support_frontiers': {
                anchor_day: {
                    ...
                    'carrier_endpoint_tau': supp.carrier_endpoint_tau,
...
def compute_cohort_maturity_rows_v3(
    ...
    # Resolve the selected-evidence seed material before display prefix
    # construction. Active and identity carrier flow through the same gate:
    # operator supply admits the denominator seed, then the generic evaluator
    # owns downstream selected mass through observed-rate K_i push-forward.
    if n_by_anchor:
