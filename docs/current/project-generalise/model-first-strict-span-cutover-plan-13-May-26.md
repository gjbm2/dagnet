...
| 5.5. **Extract algebraic spine** — introduce a small role/span algebra layer that makes the request-to-carrier-to-subject sequence visible as one contiguous transformation. It must not own frame materialisation, row schema, provenance formatting, or legacy fallback. *Outside-in gate after.* | In progress — `model_span_spine.py` landed with `resolve_request_spans` (the algebraic procedure: bind+condition → compose carrier → compose subject → compose overlays) plus post-composition projection functions. `primitive_readout.compute_resolved_runtime_readout` refactored from ~560 LOC of interleaved orchestration+diag into ~150 LOC perimeter wrapper (validate → spine → catch → result). Focused suites green (134 tests). Outside-in oracle re-run still required to close. |
