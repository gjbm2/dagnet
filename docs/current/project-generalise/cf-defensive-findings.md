...
**Status**: Active tracker, 12-May-26.
**Type**: Debt tracker — list of known violations being retired. Not agent guidance.
**Source**: [`cf-defensive-coding-audit.md`](cf-defensive-coding-audit.md) (full audit, 12-May-26, branch `feature/snapshot-db-phase0`).
**Scope**: 21 defensive-code findings in the CF engine, organised by impact, with code pointers and remediation owners.

> **Agents writing new engine code**: the rules for what to do (and not do) live in [`CF_ENGINE_DISCIPLINE.md`](../codebase/CF_ENGINE_DISCIPLINE.md). This doc is the historical inventory, not the rulebook. The list below is **debt being retired, not precedent** — do not extend any pattern shown here.

The full audit is the source of truth for severity, category breakdown, and per-finding remediation detail. This doc is the executive summary an agent reads to understand **what the engine still gets wrong and where**.

Cross-links: [INVARIANTS.md](../codebase/INVARIANTS.md) I-47 (the rule), [KNOWN_ANTI_PATTERNS.md](../codebase/KNOWN_ANTI_PATTERNS.md) AP58 (the structural pattern), [FORECAST_RUNTIME_ARCHITECTURE.md](../codebase/FORECAST_RUNTIME_ARCHITECTURE.md), [CF_PRIMITIVE_SUBSTRATE.md](../codebase/CF_PRIMITIVE_SUBSTRATE.md), [CF_ROW_PIPELINE.md](../codebase/CF_ROW_PIPELINE.md).

---

## Why these are tracked

The audit found 21 violations across `graph-editor/lib/runner/` and `bayes/compiler/`. None are blocking the current v3 rollout — they don't change answers under well-formed inputs. They all **block fail-fast diagnosis** when defective inputs arrive, and they all violate the engine-fallback rule (I-47). Several silently corrupt math (HIGH severity).
...
| M-10 | `forecast_state.py:741, 1614, 1789` (declare/global/write); `api_handlers.py:1818, 2308-2331` (read) | Module-level mutable diagnostic global `_last_forensic`. Populated as a side effect during forecast computation, then read back by the handler for the response's diagnostic envelope. Concurrent requests through the same Python process race on this slot — the second request's forensic dict can replace the first before either response is assembled. Read sites already guard with `try/except`, which papers over the race rather than naming it. Replace with a request-scoped object threaded through the call chain. |
...
|---|---|---|---|
| F-1 | `daily_conversions_derivation`, `funnel_engine`, `cohort_maturity_derivation`, `cohort_forecast_v3._selected_cohort_group_rate_draws` | Variations of `ΣY / ΣX` | Extract one mass-first reducer interface. All four feed it with appropriate evidence shapes. See [CF_HOLD_OUT_ENGINES.md](../codebase/CF_HOLD_OUT_ENGINES.md). |
...
| F-6 | `forecast_preparation.py:574-596` (preparation-layer site) vs `cohort_forecast_v3.py:1353-1376` (runtime inline fallback site) | Both call `request_envelope.build_request_envelope_plan` with the same arguments to construct the per-request envelope. The inline site is the "legacy/test entry points still work" fallback; production routes through preparation. Two construction sites with no guarantee the argument computations stay synchronised, and the inline path's failure mode degrades silently in active mode rather than refusing. Consolidate by making preparation the only construction site; tests that need a runtime-only entry point should call the preparation helper directly. See [`FORECAST_PREPARATION.md`](../codebase/FORECAST_PREPARATION.md) §5.2 and [`FORECAST_RUNTIME_ARCHITECTURE.md`](../codebase/FORECAST_RUNTIME_ARCHITECTURE.md) §10 (legacy inline fallback). |

---

## Priority remediation order (from the audit §7)

1. **H-5 — Identity carrier as data.** Single largest unification (20+ branches collapse to one), enables architecture-principle-consistent code. Substantial PR. [COHORT_ANALYSIS_NUMERATOR_DENOMINATOR_SEMANTICS.md](../codebase/COHORT_ANALYSIS_NUMERATOR_DENOMINATOR_SEMANTICS.md) invariant 6 is the canonical statement.
...
- `primitive_residual_guard.py` explicitly marks unsupported residual / complement edges rather than silently emitting zero — the model the engine should aspire to. See [CF_RESIDUAL_GUARD.md](../codebase/CF_RESIDUAL_GUARD.md).
...
- [`CF_ENGINE_DISCIPLINE.md`](../codebase/CF_ENGINE_DISCIPLINE.md) — the rules for new engine code (the doc agents must read).
- [`cf-defensive-coding-audit.md`](cf-defensive-coding-audit.md) — full audit, per-finding detail, category breakdown.
- [INVARIANTS.md](../codebase/INVARIANTS.md) I-47 — "engine fallbacks are perimeter-only" (the rule violated by every finding above).
- [INVARIANTS.md](../codebase/INVARIANTS.md) I-48 — "single conditioning locus" (the rule that H-5 fixes structurally).
- [KNOWN_ANTI_PATTERNS.md](../codebase/KNOWN_ANTI_PATTERNS.md) AP58 — "forking by case instead of degenerating one path" (the structural pattern behind H-5 and F-1 through F-5).
- [CF_PRIMITIVE_SUBSTRATE.md](../codebase/CF_PRIMITIVE_SUBSTRATE.md) §3 — the load-bearing invariants the audit measures against.
