...
The bug was caused by incorrect use of node/edge identifiers in deletion logic:

1. **Nodes have TWO identifiers:**
   - `uuid`: System-generated unique identifier (never empty)
   - `id`: Human-readable identifier (can be empty `""` for new nodes)

2. **The bug:** Deletion functions were filtering by `n.id !== id` instead of `n.uuid !== id`

3. **Why both nodes deleted:** When a newly created node has `id = ""` (empty string), the filter `n.id !== ""` evaluates to `false` for ALL nodes with empty IDs, causing ALL such nodes to be deleted instead of just one.

### Changes Made

1. **Added deletion methods to UpdateManager** (`UpdateManager.ts`):
   ...
   - Both methods properly handle UUID vs human-readable ID for edge references
   - Includes audit trail logging

2. **Updated GraphCanvas.tsx deletion functions to use UpdateManager:**
   - `handleDeleteNode`: Now uses `updateManager.deleteNode()`
   - `handleDeleteEdge`: Now uses `updateManager.deleteEdge()`
   - `deleteSelected`: Now uses UpdateManager for all deletions
   - `deleteNode` (context menu): Now uses UpdateManager
   - `deleteEdge` (context menu): Now uses UpdateManager
   - All functions now properly use `uuid` instead of `id`

3. **Fixed edge filtering bug:**
   - Line 2631: Changed `e.id !== oldEdge.id` to `e.uuid !== oldEdge.id` for cycle detection

### Benefits

1. **Correctness**: Deletions now properly use UUIDs, preventing incorrect filtering
2. **Consistency**: All deletions go through UpdateManager
3. **Edge cleanup**: UpdateManager automatically removes edges when nodes are deleted
4. **Audit trail**: All deletions are logged in UpdateManager's audit log
5. **Maintainability**: Centralized deletion logic in one place

### Testing Recommendations

1. Create a new graph with two nodes without human-readable IDs
2. Add an edge between them
3. Delete one node - verify only that node is deleted
4. Verify the edge is also removed
5. Delete the remaining node - verify graph becomes empty
6. Verify no "edge references non-existent nodes" errors
