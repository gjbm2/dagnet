...
- **CF defensive-coding audit (12-May-26)** — [docs/current/project-generalise/cf-defensive-coding-audit.md](docs/current/project-generalise/cf-defensive-coding-audit.md). 21 findings (7 HIGH / 9 MEDIUM / 5 LOW) + 5 forking/branching findings. Engine should degenerate algebraically; defense belongs at the perimeter. Top items: identity-carrier branching in `cohort_forecast_v3.py` (H-5), funnel `0.0`-substitution silently zeroing chains (H-2), monotone-repair clamp at `cohort_forecast_v3.py:3587` (H-1).
- **CF/model resolver latency authority defect (18-May-26)** — `resolve_model_params` currently reads latency from `model_vars[promoted_source].latency` and falls back to `p.latency`. CF should read the promoted latency surface only. Remove the source-ledger latency read and all fallback behaviour so stale/unpromoted model-vars cannot drive carrier/subject clocking.
...
- **Atom 3 — retire legacy reducer-side prefix** — [cohort-maturity-evidence-coverage-design.md §5.3](docs/current/cohort-maturity-evidence-coverage-design.md); **detailed step-by-step plan in [cohort-maturity-atom-3-plan.md](docs/current/cohort-maturity-atom-3-plan.md) (12-May-26)**. **Scope expanded:** audit confirmed atom 2's closure-gate parity test passes via the reducer's branch-3 silent rescue (AP59), not via the unified `SelectedAClockEvidence` path; the same slice-family filter also collapses active-carrier X coverage to 0.0 in production (Risk #8). Atom 3 is therefore not deletion-only — it must (1) make the closure-gate test assert provenance, (2) fix `n_by_anchor` for identity carrier, (3) widen the carrier-side observed-surface filter, (4) install a durable AP59 invariant guard, and only then (5) delete the rescue branches. 8 stages.
...
- **⚠ Multi-hop window/cohort parity: WEAK TESTS IN PLACE — real coverage gap** (12-May-26)
  - **Status:** Tolerances on the cohort/window collapse-parity tests have been widened far enough that the convolution arithmetic itself is no longer being meaningfully policed. Cross-mode parity regressions could slip past CI until smoother fixtures land. This is a real gap, not a nice-to-have.
  - **What got widened (and how much):**
    - `test_multihop_evidence_parity.py` Claim 1 — was 5%/5%/15% from τ=0. Now: `evidence_x` 5% from τ=3; `evidence_y` 5% from `min_tau=22`; `midpoint` 15% from `min_tau=27`. **The early-τ convolution shoulder is no longer policed at all.**
    - `test_doc56_phase0_behaviours.py::test_query_scoped_identity_carrier_collapses_public_evidence_basis` — was `pytest.approx` default rel=1e-6 (essentially exact). Now: `evidence_x` rel=0.05, `evidence_y` rel=0.10 at late τ ∈ {41,44,50,65,80}. **10% on a parity assertion is barely a test.**
    - `test_cohort_maturity_v3_contract.py::test_v3_empty_frames_cohort_mode_matches_truth_fw_curve` — checked-τ range trimmed from [12,21) to [12,19) (7 points, the test's required minimum). The oracle is a Fenton-Wilkinson moment-matched lognormal sum; FW is a known approximation that underestimates upper-tail mass when σ is large, and the runtime's selected-cohort projection now overshoots FW by ~30% at τ=19,20 while matching FW across [12,19). `model_midpoint` tracks FW across the wider range — divergence is `midpoint`-specific, consistent with exact convolution. The right long-term fix is to upgrade the oracle to an exact convolution (or a sharper approximation), then raise the upper bound.
  - **Why the widening is correct on the *current* fixture:** `synth-mirror-4step` has effective subject p≈0.077, traffic_cv=1.0, Y counts of 1–50 per τ at the convolution shoulder. The post-convolution stochastic noise on this fixture genuinely produces those drift sizes between two correct implementations; the widening is appropriate to the fixture's noise floor. The fixture is just too noisy to police the invariant sharply.
  - **Follow-up (needed to restore real coverage):** build (or pick) a smoother fixture — much higher effective p, much lower traffic_cv, larger N — and re-tighten the parity assertions from τ=0 with sub-1% tolerances. The goal is to actually exercise the convolution arithmetic at every τ on a fixture where stochastic noise is negligible. Once that fixture exists, revert the widening on the two tests above (or retire them in favour of the smooth-fixture variants).
  - **Related — parked but important:** [project-generalise](docs/current/project-generalise/). Window-mode multi-hop convolution work and the matching cross-mode contracts live there: see [multi-hop-window-evidence-rate-composition-design.md](docs/current/project-generalise/multi-hop-window-evidence-rate-composition-design.md), [multi-hop-window-evidence-rate-composition-implementation-plan.md](docs/current/project-generalise/multi-hop-window-evidence-rate-composition-implementation-plan.md), and [multi-hop-rate-composition-y-deficit-investigation.md](docs/current/project-generalise/multi-hop-rate-composition-y-deficit-investigation.md). When this thread restarts, the smooth-fixture parity work belongs alongside that effort.
  - **Related — separate but adjacent:** `test_doc56_phase0_behaviours.py::test_bayesian_sidecar_preserves_downstream_window_cohort_chart_split` is now `xfail(strict=False)` because under doc 60 WP8 the contract it polices is inverted (cohort/window asymptote collapse is now correct by design, not a defect). When WP8 lands its substrate-unification parity test, that canary needs to be either deleted or inverted — see [docs/current/project-bayes/66-shared-cf-runtime-and-wp8-admission-plan.md §8a](docs/current/project-bayes/66-shared-cf-runtime-and-wp8-admission-plan.md).
...
- **'Static' display type for frozen-snapshot scenarios** (28-Apr-26)
  ...
  - **Use case**: "show me this specific graph permanently" — share or save a chart whose numbers are pinned to a moment in time, regardless of subsequent parameter file evolution.
  - **Out of scope for 73e** (uniform materialisation is 73e's contract). Tracked here as a scope expansion. Estimated touch points: scenario type discriminator + `prepareAnalysisComputeInputs` short-circuit + capture-time strip pass + UI affordance to convert between modes.
