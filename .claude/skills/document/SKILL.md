...
- **Document integrity** — when updating an existing doc, ensure the whole document retains integrity, proportionality, and focus. Do not just splice in a narrow addition that sits awkwardly alongside existing content. Read the doc's existing structure and tone, then integrate new material so the doc reads as a coherent whole. If the new material makes an existing section out of date or contradictory, update or remove that section — do not leave stale content alongside new content. The goal is a better *document*, not a patched one.
