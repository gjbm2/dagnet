...
Mended from `_attic/test_prefix_arrival.py` per
docs/current/project-bayes/73-attic-mending-process.md. Migration:
`runner.carrier_composition.TransitionPrimitive` →
`runner.timing_span.TimingTransitionPrimitive`. Two tests tombstoned
as OBSOLETE because they referenced the deleted `compose_carrier_to_x`
oracle (audit File 5 rows 6 and 9).
"""
...
def _prim(
    ...
    *,
    ...
    """Construct a TimingTransitionPrimitive. Defaults: pure non-latency
...
@pytest.mark.skip(
    reason=(
        "OBSOLETE: references the deleted `compose_carrier_to_x` symbol "
        "on `runner.prefix_arrival`. The 73n CF generalisation removed the "
        "`runner/carrier_composition.py` module entirely; the prefix-arrival "
        "module now composes timing internally without the carrier-composer "
        "indirection. The instrumented-sentinel pattern this test embodied "
        "no longer has a target. Audit File 5 row 6 — confirmed OBSOLETE; "
        "tombstoned per 73-attic-mending-process.md §6.2."
    )
)
...
@pytest.mark.skip(
    reason=(
        "OBSOLETE: compares `arrival_weight[X]` to the deleted "
        "`compose_carrier_to_x` deterministic_cdf oracle. The "
        "`runner/carrier_composition.py` module was removed by the 73n "
        "CF generalisation (commit e15e9a9b). The reach algebra is still "
        "live but no equivalent comparator survives the deletion. The "
        "topology of the diamond is independently pinned by "
        "test_carrier_object_contract.py::test_a_not_x_topological_reach"
        "_is_product_of_upstream_probabilities and the unified prefix-"
        "arrival map. Audit File 5 row 9 — confirmed OBSOLETE; tombstoned "
...
def test_no_second_timing_path_module_imports_only_existing_layer():
    ...
    # numpy, and the existing timing_span module which itself routes
    # through span_kernel. NOT allowed: any second timing path.
    forbidden = (
    ...
    for needle in forbidden:
        assert needle not in src, (
            ...
            'span/timing composition layer (plan §605)'
        )
