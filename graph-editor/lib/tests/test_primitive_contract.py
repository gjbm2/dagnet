...
objects for each conditioning status without invoking carrier or
subject composition (plan §595). They also verify draw-family identity
(``DrawFamilyKey``-keyed RNG) at the primitive contract level. Every
constructed primitive is draw-bearing; refusal cases stop upstream at
the residual guard, not as a primitive variant.
...
def test_prior_only_primitive_has_empty_evidence_and_is_not_misreported():
    ...
    prov = primitive.to_provenance_dict()
    assert prov["status"] != "conditioned"
    # The primitive serves draws sampled from the prior (the prior is intact).
    draws = primitive.probability_draws()
    assert len(draws) == key.draw_count


# Removed: ``test_unsupported_residual_primitive_names_required_branch_and_does_not_serve_draws``
#          and ``test_unavailable_or_degraded_primitive_records_missing_inputs_and_refuses_draws``.
# ``UNSUPPORTED_RESIDUAL`` primitive construction and ``UNAVAILABLE``
# status were deleted when the engine adopted the "every constructed
# primitive is draw-bearing" invariant. Refusal now happens upstream
# at ``classify_edge_requirement`` returning ``forward_to_conditioning
# = False`` and the caller surfacing a diagnostic; the substrate never
# constructs a posterior-less primitive. ``DEGRADED`` is now
# provenance-only — see ``test_primitive_conditioning.py``'s
# degraded-topology coverage.
...
# Removed: ``test_moments_only_primitive_refuses_to_serve_a_coherent_draw_family``
#          and ``test_unavailable_status_refuses_draws_even_when_mode_is_keyed_prior``.
# Both ``MOMENTS_ONLY`` mode and ``UNAVAILABLE`` status have been
# deleted. Every constructed primitive is now draw-bearing — refusal
# happens at the residual guard before primitive construction.
...
# Removed: ``test_to_provenance_dict_unsupported_residual_carries_residual_diagnostics``.
# ``UNSUPPORTED_RESIDUAL`` status was deleted along with the
# corresponding primitive construction path. Refusal diagnostics now
# live on the ``ResidualGuardDecision`` itself
# (``rejection_reason`` + ``residual_policy``); the readout's
# ``_residual_guard_refused`` diag block carries them downstream.
