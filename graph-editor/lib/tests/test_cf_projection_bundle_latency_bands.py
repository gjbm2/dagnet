...
The daily-conversions projection bundle computes its latency-band tau set
from the resolved latency:

    inv_cdf(0.25/0.50/0.75, mu, sigma) + onset_delta_days,
    discretised with round(raw) and deduplicated by tau.

73q Phase 2 owns that derivation in one shared accessor so both reducers
and any future consumer share a single definition (73q §"Latency-band tau
accessor"). These tests pin the accessor against a local oracle and assert
that instant latency degenerates to tau 0 while long-lag bands are returned
unclamped.
"""
...
def _inline_oracle(mu, sigma, onset, *, latency_parameter=True):
    """Local oracle for the shared latency-band tau derivation."""
    band_taus = []
    for q in [0.25, 0.50, 0.75]:
        raw = (
            0.0 if not latency_parameter
            else log_normal_inverse_cdf(q, mu, sigma) + onset
        )
        tau_d = round(raw)
...
class TestLatencyBandTausShape:
    ...
    def test_instant_latency_degenerates_to_zero(self):
        # Tiny lag resolves to the same-day projection plane.
        bands = latency_band_taus(math.log(0.3), 0.1, 0.0)
        assert bands == [(0, '0d')]

    def test_structurally_non_latency_edge_degenerates_to_zero(self):
        # latency_parameter=False is authoritative: mu/sigma/onset can be
        # populated as compatibility provenance but timing is Dirac-at-zero.
        bands = latency_band_taus(
            mu=2.5,
            sigma=0.5,
            onset_delta_days=10.0,
