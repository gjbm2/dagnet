...
Tests:
  ...
  - cache miss when any load-bearing input changes: scenario_seed,
    transition identity, evidence rows, prior alpha/beta, n_effective,
    latency moments, prior_source, conditioning options
...
def _resolution(
    *,
    ...
    arrival_map = build_prefix_arrival_map(
        ...
        transitions={(src, dst): TransitionPrimitive(
