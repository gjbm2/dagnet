...
from runner.cohort_forecast_v3 import _lagless_rows, FrameEvidence
...
def test_aggregate_prior_conjugate_update_bayesian():
    ...
    rows = _lagless_rows(fe=fe, resolved=resolved, sweep_to='2026-04-01')
...
def test_class_c_fe_is_none_returns_prior():
    ...
    rows = _lagless_rows(fe=None, resolved=resolved, sweep_to='2026-04-01', axis_tau_max=20)
...
def test_class_d_returns_empty():
    ...
    rows = _lagless_rows(fe=None, resolved=resolved, sweep_to='2026-04-01')
    assert rows == []
...
def test_row_schema_matches_class_a():
    ...
    expected_fields = {
        ...
        'completeness', 'completeness_sd', 'p_infinity_mean', 'p_infinity_sd',
    }
    resolved = _make_resolved(source='bayesian', alpha=5.0, beta=5.0)
