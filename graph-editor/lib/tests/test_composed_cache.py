...
file pins is *whether the cache hooks inside* ``compose_primitive_span``
*are correctly registered, keyed, and flushed by the bustcache path* —
not whether the underlying ``ResultCache`` works in isolation (covered
in ``test_result_cache``).

The tests therefore:

  - drive the real composer entry point end-to-end with realistic
    inputs (``compose_primitive_span`` over an X→end graph with a
    populated registry and a primitive lookup that mirrors the Stage
    5b/6 readout pattern),
  - prove cache hits via object-identity on a second identical call,
  - prove cache misses when load-bearing inputs change (different
    endpoints; primitives obtained after a primitive-cache flush —
    the §417 "composed caches must invalidate when their primitives
    invalidate" invariant),
  - prove that ``snapshot_service.cache_clear`` (called from outside
    the runner package) flushes the composer cache via the shared
    registry,
  - prove that ``snapshot_service.set_cache_bypass`` propagates
    through the same ContextVar the composer cache honours,
  - prove cached and bypassed results are numerically equivalent.

Mended from `_attic/test_composed_cache.py` per
docs/current/project-bayes/73-attic-mending-process.md. Migration:
`runner.carrier_composition.{TransitionPrimitive, compose_carrier_to_x}`
→ `runner.timing_span.TimingTransitionPrimitive` (for the type) and
no replacement (for the function — entire `TestCarrierCacheWiring`
class is OBSOLETE per audit File 2 rows 1-8). The reach algebra and
cache wiring formerly tested via `compose_carrier_to_x` is now part
of the unified prefix-arrival/primitive-readout surface; the live
audit pin is `test_active_cohort_carrier_audit.py::test_active_cohort
_carrier_readout_uses_primitive_span_composer`.
"""
...
# ---------------------------------------------------------------------------
# Carrier fixture — retained for the tombstoned TestCarrierCacheWiring class.
# ---------------------------------------------------------------------------
...
def _build_subject_setup(*, alpha=2.0, beta=3.0, evidence_n=100, evidence_k=30):
    ...
    arrival_map = build_prefix_arrival_map(
        ...
        transitions={('X', 'Z'): TimingTransitionPrimitive(
...
@pytest.mark.skip(
    reason=(
        "OBSOLETE: every test in this class drives `compose_carrier_to_x` "
        "from the deleted `runner/carrier_composition.py` module. The 73n "
        "CF generalisation (commit e15e9a9b) removed that module entirely. "
        "Carrier composition now flows through the unified "
        "`compose_primitive_span` substrate; `composed_carrier` is no "
        "longer a registered cache name. The wiring this class pinned "
        "(cache hit/miss, MC-mode bypass, snapshot bustcache integration, "
        "ContextVar bypass propagation, numerical equivalence, named-cache "
        "registration) is now exercised at the unified-readout layer in "
        "test_active_cohort_carrier_audit.py and at the ResultCache "
        "contract layer in test_result_cache.py. Audit File 2 rows 1-8 — "
        "all confirmed OBSOLETE; tombstoned per "
...
class TestCarrierCacheWiring:
    ...
    def test_different_transitions_miss(self, fresh_caches):
        graph = _carrier_graph()
        a = compose_carrier_to_x(  # noqa: F821
            graph=graph, anchor_node_id='A', denominator_node_id='X',
    ...
    def test_mc_mode_does_not_serve_cached_deterministic_result(self, fresh_caches):
        ...
        graph = _carrier_graph()
        transitions = _carrier_transitions(sigma_am=0.5, sigma_mx=0.5)
        deterministic = compose_carrier_to_x(  # noqa: F821
            graph=graph, anchor_node_id='A', denominator_node_id='X',
            is_window=False, transitions=transitions, max_tau=60,
        )
        ...
        mc = compose_carrier_to_x(  # noqa: F821
            graph=graph, anchor_node_id='A', denominator_node_id='X',
            is_window=False, transitions=transitions, max_tau=60,
            num_draws=8, rng=rng,
        )
    ...
    def test_cached_and_bypassed_results_are_numerically_equivalent(self, fresh_caches):
        ...
        with result_cache.cache_bypass_ctx():
            uncached = compose_carrier_to_x(  # noqa: F821
                graph=graph, anchor_node_id='A', denominator_node_id='X',
                is_window=False, transitions=transitions, max_tau=60,
            )

        cached_first = compose_carrier_to_x(  # noqa: F821
            graph=graph, anchor_node_id='A', denominator_node_id='X',
            is_window=False, transitions=transitions, max_tau=60,
        )
        cached_second = compose_carrier_to_x(  # noqa: F821
            graph=graph, anchor_node_id='A', denominator_node_id='X',
            is_window=False, transitions=transitions, max_tau=60,
        )
...
class TestSubjectSpanCacheWiring:
    ...
    def test_different_endpoints_miss(self, fresh_caches):
        ...
        arrival_map = build_prefix_arrival_map(
            ...
            root_day_weights={'2026-03-15': 1.0},
            transitions={
                ('X', 'M'): TimingTransitionPrimitive(
                    p=0.7, mu=0.0, sigma=0.0, onset=0.0,
                    p_sd=0.0, mu_sd=0.0, sigma_sd=0.0, onset_sd=0.0,
                    source='test',
                ),
                ('M', 'Y'): TimingTransitionPrimitive(
