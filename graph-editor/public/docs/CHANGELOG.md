...
- **Refresh button on canvas analyses now reliably recommissions the analysis** — clicking Refresh in the properties panel, expression toolbar, context menu, or per-tab menu now goes through a per-analysis registry and clears every cache layer (graph compute client, per-analysis result, per-content-item, transient) before bumping the recompute nonce. Previously the click dispatched a window event that only one specific component listened for, so failures were silent and the chart appeared to redraw without actually re-running compute.
- **Auto-pull no longer silently overwrites locally-merged content on the second pull** — when an auto-pull's 3-way merge absorbed local additions (e.g. a Bayesian posterior that `applyPatch` had just written into a parameter file), the file used to be marked clean afterward, so the next auto-pull saw no local changes and quietly took the remote-wins branch — wiping the locally-merged content. Both `pullFile` and the bulk `pullLatest` now correctly preserve dirty state until the user commits, and the `dagnet:fileDirtyChanged` event reports the actual post-merge state.
- **Bayes posteriors now re-project onto live edges immediately after a fit lands** — previously, `applyPatch` and the UpdateManager cascade both projected the bare `window()` aggregate slice onto edges (neither knew the active DSL), and the DSL-aware re-projection in `useDSLReaggregation` was gated by DSL change. So after a fresh fit, edges held the wrong slice until the user happened to change DSL. `applyPatchAndCascade` now signals "posteriors landed", and `useDSLReaggregation` re-projects against the active DSL so the matching context/cohort slice appears on the canvas immediately.
...
*This entry sweeps all beta releases from v1.0 (10-Dec-25) through v1.11.x (16-Apr-26).*
