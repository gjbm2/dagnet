...
function attachShareBootConsoleGuards(page: any) {
  ...
  page.on('console', (msg: any) => {
    ...
    if (type === 'error') {
      // Transient 404s from optional BE endpoints are expected when
      // those endpoints are not deployed; callers handle them gracefully.
...
test.describe.serial('Share-live chart (persistence-first)', () => {
  ...
  test('live share (conserve-mass fixture) produces distinct scenario graphs + non-empty inbound-n (regression)', async ({ browser, baseURL }, testInfo) => {
    test.fixme(true, 'xfail pending 73q Phase 8a bridge_view direct-CF migration: assertion is on per-scenario p.mean divergence, which relied on the pre-73q write-back of p.forecast/p.evidence into p.mean. Under current semantics p.mean is the canonical blended value and is invariant across visibility_mode; visibility-mode divergence now lives in display-layer projections (funnel does this; bridge does not yet). Phase 5c decision (28-May-26) deferred the run_bridge_view refactor to Phase 8a of the 73q plan, which also owns rewriting this assertion against the new bridge output. See docs/current/project-bayes/73q-daily-conversions-shared-runtime-cutover-plan.md §"Phase 8 — Companion analysis migrations".');
