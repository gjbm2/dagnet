    ...
    async (created: Array<{ idx: number; id: string; dsl: string; scenario?: any }>) => {
      ...
      for (let i = 0; i < tabs.length; i++) {
        ...
        if (t.type !== 'chart') continue;
          ...
          let bundleMeceDimensions: string[] | undefined;
          ...
          if (bundleSnapshotMeta?.snapshotContract) {
            ...
            try {
              ...
              console.log(`[ShareBundle] Built candidate regimes for ${Object.keys(fullRegimeInventory).length} edges`);
            } catch (err) {
              console.error('[ShareBundle] Failed to build candidate regimes (degrading to BE-only resolution):', err);
            }

            try {
              bundleMeceDimensions = await computeMeceDimensions(scenarioGraphs[0].graph, workspace);
            } catch {
              // Non-blocking
            }
            ...
            for (const sg of scenarioGraphs) {
              try {
                ...
                const dslWindow = extractDateRangeFromDSL(
                  composeSnapshotDsl(bundleQueryDsl || '', effectiveQueryDsl),
                );
                if (!dslWindow) {
                ...
                console.log(`[ShareBundle] effective_query_dsl for ${sg.scenario_id}:`, effectiveQueryDsl);
                ...
                console.error(`[ShareBundle] Snapshot preparation failed for scenario ${sg.scenario_id}:`, err);
                throw err;
              }
            }
          }
