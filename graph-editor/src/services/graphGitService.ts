import { gitService, GitOperationResult } from './gitService';
...
class GraphGitService {
  ...
  async getAvailableGraphs(branch: string = this.config.branch): Promise<GraphOperationResult> {
    try {
      ...
      const files = result.data.filter((file: any) => 
        file.type === 'file' && 
  ...
  async getGraph(graphName: string, branch: string = this.config.branch): Promise<GraphOperationResult> {
    try {
  ...
  async saveGraph(
    ...
    try {
      ...
      const result = await gitService.createOrUpdateFile(
        ...
        commitMessage,
        branch,
        isUpdate ? (existingFile.data as any).sha : undefined
      );
  ...
  // Get all branches
  async getBranches(): Promise<GraphOperationResult> {
    try {
  ...
  async createBranch(
    branchName: string, 
    fromBranch: string = this.config.branch
  ): Promise<GraphOperationResult> {
    try {
  ...
  // Get repository info
  async getRepoInfo(): Promise<GraphOperationResult> {
    try {
