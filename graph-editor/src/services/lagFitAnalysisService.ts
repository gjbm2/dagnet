 ...
 */
export async function computeLagFitAnalysis(
  ...
  if (!fromTo) { console.warn(TAG, 'bail: DSL has no from()/to() —', queryDSL); return null; }
  ...
  if (!edge) { console.warn(TAG, `bail: no edge from=${fromTo.from} to=${fromTo.to}`); return null; }

  const paramId = (edge as any).p?.id;
  if (!paramId) { console.warn(TAG, 'bail: edge has no p.id', edge); return null; }

  // 4. Load parameter file (try in-memory first, fall back to IDB)
  const paramFileId = `parameter-${paramId}`;
  let paramFile = fileRegistry.getFile(paramFileId);
  if (!paramFile?.data) {
    try {
      const { db } = await import('../db/appDatabase');
      // Try unprefixed first, then scan for a workspace-prefixed key
      let idbFile: any = await db.files.get(paramFileId);
      if (!idbFile?.data) {
        const allFiles = await db.files.where('fileId').startsWith('').toArray();
        idbFile = allFiles.find((f: any) => f.fileId === paramFileId || f.fileId.endsWith(`-${paramFileId}`));
      }
      if (idbFile?.data) paramFile = idbFile;
      else console.warn(TAG, `bail: parameter file not in IDB — tried "${paramFileId}" and suffix match`);
    } catch (e) { console.warn(TAG, 'bail: IDB lookup failed', e); }
  }
  if (!paramFile?.data) { console.warn(TAG, `bail: no parameter file for "${paramFileId}"`); return null; }

  const values: ParameterValue[] = paramFile.data.values ?? [];
  const pv = bestCohortValue(values);
  if (!pv || !pv.dates?.length || !pv.n_daily?.length) {
    console.warn(TAG, `bail: no cohort value — values.length=${values.length}, bestCohort=`, pv ? { dates: pv.dates?.length, n_daily: pv.n_daily?.length } : 'none');
    return null;
  }

  // 5. Get latency meta — prefer pre-computed summary, fall back to raw daily arrays
  const latency = pv.latency ?? {};
  const rawMedianArr = pv.median_lag_days ?? [];
  const rawMeanArr = pv.mean_lag_days ?? [];

  const aggregateMedianLag: number | undefined =
    latency.median_lag_days ??
    (rawMedianArr.length > 0 ? rawMedianArr.reduce((a, b) => a + b, 0) / rawMedianArr.length : undefined);
  const aggregateMeanLag: number | undefined =
    latency.mean_lag_days ??
    (rawMeanArr.length > 0 ? rawMeanArr.reduce((a, b) => a + b, 0) / rawMeanArr.length : undefined);
  const onsetDeltaDays = latency.onset_delta_days ?? 0;
  const edgeT95 = latency.t95;

  if (!aggregateMedianLag) { console.warn(TAG, 'bail: no median lag data (neither latency summary nor daily array)'); return null; }
 ...
 */
export async function checkLagFitAvailable(graph: ConversionGraph, queryDSL: string): Promise<boolean> {
  return (await computeLagFitAnalysis(graph, queryDSL)) !== null;
}
