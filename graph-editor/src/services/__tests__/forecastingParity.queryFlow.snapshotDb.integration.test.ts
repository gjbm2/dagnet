...
function findRepoRoot(): string {
  let dir = process.cwd();
  for (let i = 0; i < 10; i++) {
    if (fs.existsSync(path.join(dir, 'graph-editor')) && fs.existsSync(path.join(dir, 'debug'))) return dir;
...
describe('Forecasting parity — query flow + snapshot DB (integration)', () => {
  ...
  const PARAM_REL = 'debug/graph-bundles/<private-repo>__conversion-flow-v2-recs-collapsed/parameters/registration-to-success.yaml';
  const PARAM_ID = 'registration-to-success';
  const CHANNEL_CONTEXT_REL = 'debug/graph-bundles/<private-repo>__conversion-flow-v2-recs-collapsed/contexts/channel.yaml';
