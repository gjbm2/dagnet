...
// ===========================================================================
// CHANGE 2: BE topo pass — verify sequential ordering (tested via mock)
// ===========================================================================

describe('Change 2: BE topo pass ordering', () => {
  it('setGraph is called with graph containing BOTH FE and BE mutations (no race)', async () => {
    await seedTestData(GID);

    // Track all setGraph calls
    const setGraphCalls: any[] = [];

    vi.mocked(executeRetrieveAllSlicesWithProgressToast).mockImplementation(
      async (options: any) => {
        const wrappedSetGraph = (g: any) => {
        ...
        // Simulate FE LAG writing analytic entry
        mutated.edges[0].p.model_vars = [{
          source: 'analytic',
          probability: { mean: 0.52 },
          latency: { mu: 5.1 },
        }];
        wrappedSetGraph(mutated);

        // Simulate BE topo writing analytic_be entry to the SAME graph
        // (Change 2 means this happens sequentially, not fire-and-forget)
        mutated.edges[0].p.model_vars.push({
          source: 'analytic_be',
          probability: { mean: 0.51 },
          latency: { mu: 5.0 },
        });
        wrappedSetGraph(mutated);

        // Simulate inbound-n — should see BOTH analytic + analytic_be
        mutated.edges[0].p.n = 1000;
        wrappedSetGraph(mutated);
    ...
    // Final graph should have BOTH model_vars entries AND inbound-n
    const final = fileRegistry.getFile(GID)?.data as any;
    expect(final.edges[0].p.model_vars).toHaveLength(2);
    expect(final.edges[0].p.model_vars[0].source).toBe('analytic');
    expect(final.edges[0].p.model_vars[1].source).toBe('analytic_be');
    expect(final.edges[0].p.n).toBe(1000);

    // The last setGraph call should contain all three mutations
    const lastCall = setGraphCalls[setGraphCalls.length - 1];
    expect(lastCall.edges[0].p.model_vars).toHaveLength(2);
    expect(lastCall.edges[0].p.n).toBe(1000);
  });
});
