...
describe('UpdateManager.updateConditionalProbability evidence window metadata', () => {
  it('writes evidence.scope_from/scope_to (normalised to UK) on conditional_p', () => {
    ...
    const updated = um.updateConditionalProbability(
      ...
      0,
      {
        evidence: {
          n: 10,
          k: 3,
          scope_from: '2025-11-01T00:00:00.000Z',
          scope_to: '2025-11-14T00:00:00.000Z',
    ...
    expect(updated.edges[0].conditional_p[0].p.evidence.scope_from).toBe('1-Nov-25');
    expect(updated.edges[0].conditional_p[0].p.evidence.scope_to).toBe('14-Nov-25');
