...
describe('Dirty State Tracking', () => {
  ...
  describe('Initialization Period', () => {
    it('should NOT mark dirty during initialization period', async () => {
      const graph = createTestGraph();
      const file = await fileRegistry.getOrCreateFile(
        'graph-test',
        'graph',
      ...
      // File is still initializing
      expect(file.isInitializing).toBe(true);

      // Make a change during initialization
      const modifiedGraph = structuredClone(graph);
      modifiedGraph.nodes[0].label = 'Changed During Init';
      await fileRegistry.updateFile('graph-test', modifiedGraph);

      // Should NOT be dirty during init
      expect(file.isDirty).toBe(false);
      
      // But data should be updated
      expect(file.data.nodes[0].label).toBe('Changed During Init');
      
      // And originalData should also be updated (to match current)
      expect(file.originalData.nodes[0].label).toBe('Changed During Init');
    });

    it('should mark dirty for STRUCTURAL changes during initialization (adding nodes)', async () => {
      const graph = createTestGraph();
      const file = await fileRegistry.getOrCreateFile(
        'graph-test',
        'graph',
      ...
      // Make a STRUCTURAL change during initialization (add a node)
      const modifiedGraph = structuredClone(graph);
      modifiedGraph.nodes.push({
        uuid: 'node-new',
        id: 'new-node',
      ...
      // SHOULD be dirty because structural change ends initialization
      expect(file.isDirty).toBe(true);
      expect(file.isInitializing).toBe(false);
