...
class WorkspaceService {
  ...
  async cloneWorkspace(repository: string, branch: string, gitCreds: any): Promise<WorkspaceState> {
    ...
    try {
      ...
      // Sort by priority: if rate limits cut the clone short, the most useful files
      // are fetched first (graphs for exploration, indexes for navigator display).
      const CLONE_TYPE_PRIORITY: Record<string, number> = {
        graph: 0, 'hash-mappings': 1, settings: 2,
        node: 3, event: 4, context: 5, case: 6, parameter: 7
      };
      filesToFetch.sort((a, b) => {
        const aIsIndex = a.dirConfig.isIndex ? -1 : 0;
        const bIsIndex = b.dirConfig.isIndex ? -1 : 0;
        const aPri = (CLONE_TYPE_PRIORITY[a.dirConfig.type] ?? 99) + aIsIndex;
        const bPri = (CLONE_TYPE_PRIORITY[b.dirConfig.type] ?? 99) + bIsIndex;
        return aPri - bPri;
      });
      ...
      // Warn if any files failed to fetch
      const failedCount = filesToFetch.length - fileIds.length;
      if (failedCount > 0) {
        const failedPaths = filesToFetch
          .map(f => f.treeItem.path)
          .filter(path => !fileIds.some(id => path.endsWith(id.replace(/^[^-]+-/, '') + '.yaml') || path.endsWith(id.replace(/^[^-]+-/, '') + '.json')));
        console.warn(`⚠️ WorkspaceService: ${failedCount} of ${filesToFetch.length} files failed to fetch:`, failedPaths);
        sessionLogService.warning('git', 'GIT_CLONE_PARTIAL',
          `Clone incomplete: ${failedCount} of ${filesToFetch.length} files could not be fetched`,
          failedPaths.join(', '),
          { repository, branch, failedCount, totalCount: filesToFetch.length }
        );
        if (typeof window !== 'undefined') {
          window.dispatchEvent(new CustomEvent('dagnet:clonePartialFailure', {
            detail: { failedCount, totalCount: filesToFetch.length, successCount: fileIds.length }
          }));
        }
      }
  ...
  /**
   * Hydrate workspace from a pre-built sample-data bundle (zero GitHub API calls).
   *
   * The bundle is a JSON file generated by scripts/bundle-sample-data.js and served
   * from public/defaults/sample-data-bundle.json.  It contains every file's parsed
   * data and git blob SHA, allowing subsequent pulls to SHA-diff correctly.
   */
  async hydrateFromBundle(
    repository: string,
    branch: string,
    bundle: {
      commitSha: string;
    ...
    const workspaceId = `${repository}-${branch}`;
    console.log(`📦 WorkspaceService: Hydrating workspace ${workspaceId} from static bundle (${bundle.files.length} files)...`);

    const logOpId = sessionLogService.startOperation(
      'info',
      'workspace',
      'WORKSPACE_HYDRATE_BUNDLE',
      `Hydrating workspace ${repository}/${branch} from sample data bundle`,
      { repository, branch, fileCount: bundle.files.length }
    );

    // Clear any existing workspaces (single workspace policy)
    const existingWorkspaces = await db.workspaces.count();
    if (existingWorkspaces > 0) {
      await this.clearAllWorkspaces();
    }
    ...
    for (const bundleFile of bundle.files) {
      try {
        let fileModTime = Date.now();
        if (bundleFile.data?.metadata) {
          if (bundleFile.data.metadata.updated_at) {
            fileModTime = new Date(bundleFile.data.metadata.updated_at).getTime();
          } else if (bundleFile.data.metadata.created_at) {
            fileModTime = new Date(bundleFile.data.metadata.created_at).getTime();
          }
        }

        const fileState: FileState = {
          fileId: bundleFile.fileId,
          type: bundleFile.type as ObjectType,
          name: bundleFile.name,
          path: bundleFile.path,
          data: bundleFile.data,
          originalData: structuredClone(bundleFile.data),
          isDirty: false,
          isInitializing: true,
          source: {
            repository,
        ...
        const idbFileId = `${repository}-${branch}-${bundleFile.fileId}`;
        await db.files.put({ ...fileState, fileId: idbFileId });
        (fileRegistry as any).files.set(bundleFile.fileId, fileState);
        fileIds.push(bundleFile.fileId);
      } catch (error) {
        console.error(`❌ WorkspaceService: Failed to hydrate ${bundleFile.path}:`, error);
      }
    }
    ...
    for (const [type, count] of filesByType.entries()) {
      sessionLogService.addChild(logOpId, 'success', `HYDRATE_${type.toUpperCase()}`, `Hydrated ${count} ${type} file(s)`);
    }
    ...
    const typeSummary = Array.from(filesByType.entries()).map(([t, c]) => `${t}: ${c}`).join(', ');
    console.log(`⚡ WorkspaceService: Bundle hydration complete in ${elapsed}ms! ${fileIds.length} files (${typeSummary})`);

    sessionLogService.endOperation(logOpId, 'success', `Hydrated ${fileIds.length} files from bundle in ${elapsed}ms`, { repository, branch, filesAffected: fileIds, duration: elapsed });
  ...
  async pullLatest(repository: string, branch: string, gitCreds: any, options?: PullOptions): Promise<PullResult> {
    ...
    try {
      ...
      if (toFetch.length === 0 && toDelete.length === 0) {
        // Still fetch images even if no text file changes (reuse tree)
      ...
      // STEP 8: Fetch and store images (reuse tree)
      console.log(`🖼️ WorkspaceService: Fetching images...`);
      const images = await this.fetchAllImagesFromGit(repository, branch, gitCreds, tree);
  ...
  async fetchAllImagesFromGit(
    ...
    console.log(`🖼️ WorkspaceService.fetchAllImagesFromGit: Checking ${imagesPath} (branch: ${branch}, tree provided: ${!!tree})`);
    
    try {
      // Discover image files — prefer the already-fetched tree (saves 1 API call)
      let files: Array<{ name: string; sha: string }>;

      if (tree) {
        const IMAGE_RE = /\.(png|jpg|jpeg|gif|webp)$/i;
        const prefix = imagesPath + '/';
        files = tree
          .filter((item: any) => item.type === 'blob' && item.path.startsWith(prefix) && IMAGE_RE.test(item.path))
          .map((item: any) => ({ name: item.path.split('/').pop()!, sha: item.sha }));
      } else {
        ...
        files = response.data
          .filter((item: any) => item.type === 'file' && /\.(png|jpg|jpeg|gif|webp)$/i.test(item.name))
          .map((item: any) => ({ name: item.name, sha: item.sha }));
      }

      if (files.length === 0) {
        console.log(`🖼️ WorkspaceService: No image files found at ${imagesPath}`);
        return allImages;
      }
      ...
      const results = await Promise.all(
          files.map(async (file: { name: string; sha: string }) => {
            try {
