 ...
 *
 * The `yaml` package v1.x pushes duplicate-key violations into `doc.errors`
 * and `YAML.parse()` unconditionally throws the first error.  Remote index
 * files (generated by the Python pipeline) legitimately contain duplicate
 * `created_at` keys.  This helper uses `parseDocument` so we can filter
 * those errors out and only throw on genuine parse failures.
 */
function parseYamlLenient(src: string): any {
  const doc = YAML.parseDocument(src);
  const realErrors = doc.errors.filter(
    (e: any) => !(e.message && e.message.includes('Map keys must be unique'))
  );
      ...
      const fetchSingleFile = async ({ treeItem, dirConfig }: { treeItem: any, dirConfig: any }): Promise<string | null> => {
        try {
          ...
          const contentStr = blobResult.data.content;
            ...
            } else if (dirConfig.extension === 'json') {
              try {
                data = JSON.parse(contentStr);
              } catch {
      ...
      if (toFetch.length > 0) {
        ...
        const fetchPromises = toFetch.map(async ({ treeItem, dirConfig, isNew }) => {
          try {
            ...
            let data: any;
            if (dirConfig.type === 'graph') {
              // Graph files are always JSON.
              data = JSON.parse(contentStr);
            } else if (dirConfig.extension === 'json') {
              // Nominally JSON, but some root files (e.g. hash-mappings.json)
              // may actually contain YAML.  Try JSON first, fall back to YAML.
              try {
                data = JSON.parse(contentStr);
              } catch {
                data = parseYamlLenient(contentStr);
              ...
              // YAML files — lenient parse tolerates duplicate keys from
              // Python pipeline (e.g. created_at appears twice per entry).
              data = parseYamlLenient(contentStr);
            }
              ...
              if (isDerivedDataFile) {
                ...
                if (forceReplaceRequested) {
                  if (forceReplaceMode === 'apply' && allowForceReplace.has(fileId)) {
                    ...
                    // Always update FileRegistry in-memory cache (matches clean-update path).
                    // Without this, IDB is updated but FileRegistry retains stale data,
                    // causing the planner and other consumers to see outdated file content.
            ...
            console.error(`❌ WorkspaceService: Failed to update ${treeItem.path}: ${errMsg}`);
          }
        });
