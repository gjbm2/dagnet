  ...
  const loadCredentialsAndUpdateRepo = async () => {
    try {
      ...
      if (result.success && result.credentials) {
        const defaultRepo = result.credentials.defaultGitRepo || '<private-repo>';
  ...
  const loadStateFromDB = async () => {
    ...
    if (appState?.navigatorState) {
      const restoredState = {
        ...
        searchQuery: '',
        selectedRepo: appState.navigatorState.selectedRepo || '<private-repo>',
