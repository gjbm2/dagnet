export function toFlow(graph, callbacks) {
    if (!graph)
        return { nodes: [], edges: [] };
...
export function fromFlow(nodes, edges, original) {
    if (!original)
        return null;
    return {
        ...original,
        ...
        edges: edges.map((e) => {
            ...
            const originalEdge = original.edges?.find((oe) => oe.id === e.id || `${oe.from}->${oe.to}` === e.id);
            return {
