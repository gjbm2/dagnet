 ...
 */
export function resolveNodeRefToId(graph, ref) {
    if (!graph?.nodes)
        return ref;
    ...
    if (byId)
        return byId.id;
    // Try to find by slug
    const bySlug = graph.nodes.find((n) => n.slug === ref);
    if (bySlug)
        return bySlug.id;
 ...
 */
function getImplicitlyVisitedNodes(graph, whatIfOverrides, legacyWhatIfAnalysis) {
    ...
    whatIfOverrides?.caseOverrides?.forEach((selectedVariant, caseNodeId) => {
        ...
        graph.edges.forEach((edge) => {
            if (edge.case_id && edge.from) {
                const caseNode = graph.nodes.find((n) => n.id === edge.from && n.case?.id === edge.case_id);
    ...
    if (legacyWhatIfAnalysis?.caseNodeId && legacyWhatIfAnalysis?.selectedVariant) {
        ...
        const legacyCaseNode = graph.nodes.find((n) => n.id === legacyWhatIfAnalysis.caseNodeId);
 ...
 */
export function computeEffectiveEdgeProbability(graph, edgeId, whatIfOverrides, legacyWhatIfAnalysis, givenVisitedNodes) {
    if (!graph?.edges)
        return 0;
    // Find the edge in the graph
    const edge = graph.edges.find((e) => e.id === edgeId || `${e.from}->${e.to}` === edgeId);
    if (!edge)
        return 0;
    ...
    if (edge.conditional_p && edge.conditional_p.length > 0) {
        ...
        if (override && override.size > 0) {
            ...
            for (const conditionalProb of edge.conditional_p) {
                if (!conditionalProb?.condition?.visited)
                    continue;
                // Resolve all condition node references to IDs
                const conditionNodeIds = conditionalProb.condition.visited.map((ref) => resolveNodeRefToId(graph, ref));
                // Check if override matches this condition
                const matches = conditionNodeIds.length === override.size &&
                    conditionNodeIds.every((nodeId) => override.has(nodeId));
                if (matches) {
            ...
            if (allVisitedNodes.size > 0) {
                for (const conditionalProb of edge.conditional_p) {
                    ...
                    const allVisited = conditionNodeIds.length > 0 &&
                        conditionNodeIds.every((nodeId) => allVisitedNodes.has(nodeId));
                    if (allVisited) {
    ...
    if (edge.case_id && edge.case_variant) {
        // Find the case node
        const caseNode = graph.nodes.find((n) => n.type === 'case' && n.case?.id === edge.case_id);
 ...
 */
export function getEdgeWhatIfDisplay(graph, edgeId, whatIfOverrides, legacyWhatIfAnalysis) {
    if (!graph?.edges)
        return null;
    const edge = graph.edges.find((e) => e.id === edgeId || `${e.from}->${e.to}` === edgeId);
    if (!edge)
        ...
        if (override && override.size > 0) {
            ...
            if (implicitlyVisited.size > 0) {
                for (const conditionalProb of edge.conditional_p) {
                    ...
                    const conditionNodeIds = conditionalProb.condition.visited.map((ref) => resolveNodeRefToId(graph, ref));
                    const allVisited = conditionNodeIds.length > 0 &&
                        conditionNodeIds.every((nodeId) => implicitlyVisited.has(nodeId));
                    if (allVisited) {
                        return {
                            type: 'conditional',
                            probability: conditionalProb.p?.mean ?? 0,
