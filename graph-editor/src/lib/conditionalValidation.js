 ...
 */
export function validateConditionalProbabilities(graph) {
    const errors = [];
    const warnings = [];
    // For each node
    for (const node of graph.nodes) {
                    ...
                    const condProbSum = variantEdges.reduce((sum, edge) => {
                        if (edge.conditional_p) {
                            for (const cp of edge.conditional_p) {
                                const resolvedConditionNodes = cp.condition.visited.map(ref => resolveNodeReference(ref, graph.nodes) || ref);
        ...
        for (const conditionNodeIdOrSlug of conditions) {
            ...
            const condProbSum = calculateConditionalProbabilitySum(outgoingEdges, [conditionNodeId], graph.nodes);
            if (Math.abs(condProbSum - 1.0) > PROB_SUM_TOLERANCE) {
                errors.push({
                    type: 'probability_sum',
                    nodeId: node.id,
 ...
 */
function resolveNodeReference(nodeIdOrSlug, nodes) {
    // Try ID first
 ...
 */
function collectUniqueConditions(edges, nodes) {
 ...
 */
function calculateBaseProbabilitySum(edges, nodes) {
 ...
 */
function calculateConditionalProbabilitySum(edges, visitedNodes, nodes) {
    return edges.reduce((sum, edge) => {
        // Find matching conditional probability
        if (edge.conditional_p) {
            for (const cp of edge.conditional_p) {
                // Check if all nodes in condition are in visitedNodes (resolve slugs to IDs)
                const resolvedConditionNodes = cp.condition.visited.map(ref => resolveNodeReference(ref, nodes) || ref);
 ...
 */
function isUpstream(targetNodeId, sourceNodeId, graph) {
 ...
 */
function hasCircularDependency(fromNode, targetNode, originalNode, graph, visited) {
 ...
 */
export function getUpstreamNodes(edgeFromNodeId, graph) {
    const upstreamNodeIds = new Set();
    const visited = new Set();
    const queue = [edgeFromNodeId];
    while (queue.length > 0) {
        const currentId = queue.shift();
        if (visited.has(currentId))
            continue;
 ...
 */
export function getEffectiveProbability(edge, visitedNodes, nodes) {
