 ...
 */
function findTrackedNodes(graph) {
 ...
 */
function getEffectiveEdgeProbability(edge, visitedSet, graph, options) {
 ...
 */
function serializeVisitedSet(visitedSet, trackedNodes) {
 ...
 */
function parseVisitedSet(serialized) {
    if (!serialized)
        return new Set();
 ...
 */
export function calculateProbabilities(graph, options) {
    ...
    const edgeTraverseProb = new Map();
    ...
    for (const nodeId of sortedNodes) {
        ...
        const nodeStates = Array.from(states.entries()).filter(([key]) => key.startsWith(`${nodeId}|`));
        // For each state (nodeId, visitedSet) with probability P
        for (const [stateKey, prob] of nodeStates) {
            if (prob === 0)
                continue; // Skip zero-probability states
            ...
            for (const edge of outgoingEdges) {
                ...
                if (edgeProb === 0)
                    continue; // Skip zero-probability edges
    ...
    for (const e of graph.edges) {
        const monetaryCost = e.costs?.monetary;
        const timeCost = e.costs?.time;
        const edgeCost = {
            monetary: typeof monetaryCost === 'object' && monetaryCost !== null ? (monetaryCost.value || 0) : (typeof monetaryCost === 'number' ? monetaryCost : 0),
            time: typeof timeCost === 'object' && timeCost !== null ? (timeCost.value || 0) : (typeof timeCost === 'number' ? timeCost : 0)
        };
    ...
    for (const node of graph.nodes) {
        ...
        for (const e of graph.edges) {
            const edgeReach = reachableFrom.get(e.to) || new Set();
