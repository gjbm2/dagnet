...
export async function loadFromSheet() {
    const p = new URLSearchParams(window.location.search);
    const sheet = p.get('sheet');
    const tab = p.get('tab') || 'Graphs';
    const row = p.get('row');
    if (!APPS_SCRIPT || !sheet || !row)
        return null; // allow local editing via ?data= or paste
...
export async function saveToSheet(graph) {
    const p = new URLSearchParams(window.location.search);
    const sheet = p.get('sheet');
    const tab = p.get('tab') || 'Graphs';
    const row = p.get('row');
    if (!APPS_SCRIPT || !sheet || !row)
        throw new Error('Missing sheet/tab/row or APPS_SCRIPT url');
