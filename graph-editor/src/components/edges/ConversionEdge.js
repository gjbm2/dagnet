...
export default function ConversionEdge({ id, sourceX, sourceY, targetX, targetY, sourcePosition, targetPosition, data, selected, source, target, }) {
    ...
    const [adjustedLabelPosition, setAdjustedLabelPosition] = useState(null);
    ...
    const getTooltipContent = () => {
        ...
        if (data.case_variant) {
            ...
            if (data.case_id) {
                ...
                if (sourceNode?.type === 'case' && sourceNode?.case?.id === data.case_id) {
                    const variant = sourceNode.case.variants?.find((v) => v.name === data.case_variant);
                    if (variant) {
        ...
        if (fullEdge?.conditional_p && fullEdge.conditional_p.length > 0) {
            ...
            for (const cond of fullEdge.conditional_p) {
                const nodeNames = cond.condition.visited.map((nodeId) => {
    ...
    const fullEdge = graph?.edges.find((e) => e.id === id);
    ...
    const getBezierPoint = (t, sx, sy, c1x, c1y, c2x, c2y, ex, ey) => {
        const mt = 1 - t;
        const mt2 = mt * mt;
        const mt3 = mt2 * mt;
        const t2 = t * t;
    ...
    const calculateBezierLengthToT = (sx, sy, c1x, c1y, c2x, c2y, ex, ey, tMax) => {
        const steps = 100;
        let length = 0;
    ...
    const calculateBezierLength = (sx, sy, c1x, c1y, c2x, c2y, ex, ey) => {
    ...
    const arrowPositions = React.useMemo(() => {
        ...
        try {
            ...
            const findTForArcLength = (targetLength) => {
                if (targetLength <= 0)
                    return 0;
                if (targetLength >= L)
                    return 1;
            ...
            function calculatePointOnCurve(t, sx, sy, c1x, c1y, c2x, c2y, ex, ey, lerp) {
    ...
    React.useEffect(() => {
        ...
        const timeoutId = setTimeout(() => {
            ...
            const rectanglesOverlap = (x1, y1, w1, h1, x2, y2, w2, h2) => {
            ...
            const checkLabelCollisions = (lx, ly) => {
                let collisions = 0;
                for (const edge of otherEdges) {
                    ...
                    for (const labelEl of Array.from(labelElements)) {
                        ...
                        if (rect.width > 0 && rect.height > 0) {
                            ...
                            if (rectanglesOverlap(lx, ly, labelWidth, labelHeight, rect.left + rect.width / 2, rect.top + rect.height / 2, rect.width, rect.height)) {
                                collisions++;
                            }
                        }
                    }
            ...
            const checkNodeCollisions = (lx, ly) => {
                let collisions = 0;
                for (const node of nodes) {
                    const nodeWidth = node.data?.type === 'case' ? 96 : 120;
                    const nodeHeight = node.data?.type === 'case' ? 96 : 120;
                    const nodeX = node.position.x + nodeWidth / 2;
                    const nodeY = node.position.y + nodeHeight / 2;
                    if (rectanglesOverlap(lx, ly, labelWidth, labelHeight, nodeX, nodeY, nodeWidth, nodeHeight)) {
                        collisions++;
                    }
                }
            ...
            const checkEdgePathCollisions = (lx, ly) => {
                let collisions = 0;
                for (const edge of otherEdges) {
    ...
    const handleSourceMouseDown = useCallback((e) => {
        e.preventDefault();
        e.stopPropagation();
    ...
    return (_jsxs(_Fragment, { children: [_jsx("defs", { children: _jsx("marker", { id: `arrow-${id}`, markerWidth: "10", markerHeight: "10", refX: "9", refY: "3", orient: arrowPosition.angle, markerUnits: "strokeWidth", children: _jsx("path", { d: "M0,0 L0,6 L9,3 z", fill: getEdgeColor() }) }) }), _jsx("path", { id: id, style: {
                    stroke: getEdgeColor(),
                ...
                }, className: "react-flow__edge-path", d: edgePath, onContextMenu: handleContextMenu, onDoubleClick: handleDoubleClick }), _jsx("path", { id: `${id}-selection`, style: {
                    stroke: 'transparent',
                    strokeWidth: 20,
                    fill: 'none',
                    zIndex: selected ? 1000 : 1,
                }, className: "react-flow__edge-path", d: edgePath, onDoubleClick: handleDoubleClick }), data?.calculateWidth && arrowPositions.map((pos, index) => (_jsx("g", { children: _jsx("polygon", { points: `${pos.x - 6},${pos.y - 4} ${pos.x - 6},${pos.y + 4} ${pos.x + 6},${pos.y}`, fill: "#f8f9fa", stroke: getEdgeColor(), strokeWidth: "1", transform: `rotate(${pos.angle} ${pos.x} ${pos.y})`, style: { zIndex: selected ? 1000 : 1001 } }) }, index))), !data?.calculateWidth && (_jsx("polygon", { points: `${arrowPosition.x - 4},${arrowPosition.y - 3} ${arrowPosition.x - 4},${arrowPosition.y + 3} ${arrowPosition.x + 4},${arrowPosition.y}`, fill: getEdgeColor(), transform: `rotate(${arrowPosition.angle} ${arrowPosition.x} ${arrowPosition.y})`, style: { zIndex: selected ? 1000 : 1 } })), _jsxs(EdgeLabelRenderer, { children: [_jsx("div", { style: {
                            position: 'absolute',
                        ...
                        }, onDoubleClick: handleDoubleClick, onMouseEnter: (e) => {
                            const rect = e.currentTarget.getBoundingClientRect();
                            setTooltipPos({ x: rect.left + rect.width / 2, y: rect.top });
                            setShowTooltip(true);
                        }, onMouseLeave: () => {
                            setShowTooltip(false);
                        }, children: _jsxs("div", { style: { textAlign: 'center' }, children: [(data?.probability === undefined || data?.probability === null) ? (_jsx("div", { style: {
                                        fontWeight: 'bold',
                                        color: '#ff6b6b',
                                        fontSize: '11px',
                                        background: '#fff5f5',
                                    ...
                                    }, children: "\u26A0\uFE0F No Probability" })) : whatIfDisplay?.isOverridden ? (
                                // UNIFIED: Any what-if override (conditional or case variant)
                                _jsxs("div", { style: { display: 'flex', flexDirection: 'column', gap: '2px' }, children: [_jsxs("div", { style: {
                                                fontWeight: 'bold',
                                                fontSize: '11px',
                                                ...
                                                padding: '2px 4px',
                                                borderRadius: '2px'
                                            }, children: [Math.round((effectiveProbability || 0) * 100), "%"] }), _jsx("div", { style: {
                                                fontWeight: 'normal',
                                                fontSize: '9px',
                                                color: '#666'
                                            }, children: whatIfDisplay.displayLabel || '🔬 What-If' })] })) : isCaseEdge ? (
                                ...
                                _jsxs("div", { style: { display: 'flex', flexDirection: 'column', gap: '2px' }, children: [_jsxs("div", { style: {
                                                ...
                                                fontSize: '10px',
                                                color: '#666'
                                            }, children: "variant weight" })] })) : (_jsxs("div", { style: { fontWeight: 'bold' }, children: [Math.round((effectiveProbability || 0) * 100), "%", data?.stdev && data.stdev > 0 && (_jsxs("span", { style: { fontSize: '10px', color: '#666', marginLeft: '4px' }, children: ["\u00B1", Math.round(data.stdev * 100), "%"] }))] })), data?.costs && (data.costs.monetary || data.costs.time) && (_jsxs("div", { style: { fontSize: '10px', color: '#666', marginTop: '2px' }, children: [data.costs.monetary && (_jsxs("div", { children: ["\u00A3", typeof data.costs.monetary === 'object' ? data.costs.monetary.value : data.costs.monetary, typeof data.costs.monetary === 'object' && data.costs.monetary.currency && ` ${data.costs.monetary.currency}`] })), data.costs.time && (_jsxs("div", { children: [typeof data.costs.time === 'object' ? data.costs.time.value : data.costs.time, typeof data.costs.time === 'object' ? (data.costs.time.units || 'days') : 'days'] }))] })), data?.description && (_jsx("div", { style: {
                                        fontSize: '9px',
                                        color: '#888',
                                        marginTop: '2px',
                                        fontStyle: 'italic',
                                        ...
                                        textAlign: 'center',
                                        lineHeight: '1.2'
                                    }, children: data.description }))] }) }), selected && (_jsx(_Fragment, { children: _jsx("div", { style: {
                                position: 'absolute',
                                ...
                                zIndex: 1000,
                                pointerEvents: 'none',
                            }, title: "Delete edge (use context menu)", children: "\u00D7" }) }))] }), showTooltip && ReactDOM.createPortal(_jsx("div", { style: {
                    position: 'fixed',
                    ...
                    zIndex: 10000,
                    pointerEvents: 'none',
                }, children: getTooltipContent() }), document.body), showContextMenu && (_jsxs("div", { style: {
                    position: 'absolute',
                    transform: `translate(-50%, -50%) translate(${finalLabelX}px,${finalLabelY}px)`,
                    background: '#fff',
                    ...
                    minWidth: '120px',
                        ...
                        }, onClick: handleReconnectSource, onMouseEnter: (e) => e.currentTarget.style.background = '#f8f9fa', onMouseLeave: (e) => e.currentTarget.style.background = '#fff', children: "Reconnect Source" }), _jsx("div", { style: {
                            padding: '8px 12px',
                            cursor: 'pointer',
                            fontSize: '12px',
                            borderBottom: '1px solid #eee',
                        }, onClick: handleReconnectTarget, onMouseEnter: (e) => e.currentTarget.style.background = '#f8f9fa', onMouseLeave: (e) => e.currentTarget.style.background = '#fff', children: "Reconnect Target" }), _jsx("div", { style: {
                            padding: '8px 12px',
                            cursor: 'pointer',
                            fontSize: '12px',
                            color: '#dc3545',
                            ...
                            handleDelete();
                            setShowContextMenu(false);
                        }, onMouseEnter: (e) => e.currentTarget.style.background = '#f8f9fa', onMouseLeave: (e) => e.currentTarget.style.background = '#fff', children: "Delete Edge" })] }))] }));
}
