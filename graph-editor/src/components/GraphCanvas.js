...
import ReactFlow, { Background, Controls, MiniMap, useNodesState, useEdgesState, ReactFlowProvider, useReactFlow, Panel, ConnectionMode, } from 'reactflow';
...
export default function GraphCanvas({ onSelectedNodeChange, onSelectedEdgeChange, onDoubleClickNode, onDoubleClickEdge, onSelectEdge, edgeScalingMode, autoReroute }) {
    return (_jsx(ReactFlowProvider, { children: _jsx(CanvasInner, { onSelectedNodeChange: onSelectedNodeChange, onSelectedEdgeChange: onSelectedEdgeChange, onDoubleClickNode: onDoubleClickNode, onDoubleClickEdge: onDoubleClickEdge, onSelectEdge: onSelectEdge, edgeScalingMode: edgeScalingMode, autoReroute: autoReroute }) }));
}
function CanvasInner({ onSelectedNodeChange, onSelectedEdgeChange, onDoubleClickNode, onDoubleClickEdge, onSelectEdge, edgeScalingMode, autoReroute }) {
    ...
    // Auto-layout state
    const [preLayoutGraph, setPreLayoutGraph] = useState(null);
    ...
    const logMassTransform = useCallback((probability, maxWidth) => {
        if (probability <= 0)
            return 0;
        if (probability >= 1)
            return maxWidth;
    ...
    const MIN_WIDTH = 2;
    const calculateEdgeWidth = useCallback((edge, allEdges, allNodes) => {
        ...
        const getEffectiveProbability = (e) => {
        ...
        if (edgeScalingMode === 'global-mass') {
            ...
            const startNode = allNodes.find(n => n.data?.entry?.is_start === true || (n.data?.entry?.entry_weight || 0) > 0);
            if (!startNode) {
        ...
        function calculateResidualProbability(targetNode, allEdges, startNode) {
            ...
            const residualAtNode = {};
            const visiting = new Set();
            function dfs(nodeId) {
                if (residualAtNode[nodeId] !== undefined)
                    return residualAtNode[nodeId];
    ...
    const getEdgeSortKey = useCallback((sourceNode, targetNode, face, isSourceFace = true) => {
    ...
    const calculateEdgeOffsets = useCallback((edgesWithWidth, allNodes, maxWidth) => {
    ...
    const performAutoReroute = useCallback(() => {
        ...
        const edgesToReroute = edges.filter(edge => movedNodes.includes(edge.source) || movedNodes.includes(edge.target));
    ...
    const getAllExistingSlugs = useCallback((excludeId) => {
        if (!graph)
            return [];
    ...
    const pendingReconnectionRef = useRef(null);
    ...
    const onEdgeUpdate = useCallback((oldEdge, newConnection) => {
        ...
        reconnectionTimeoutRef.current = setTimeout(() => {
            ...
            const nextGraph = structuredClone(graph);
            const edgeIndex = nextGraph.edges.findIndex((e) => e.id === oldEdge.id);
            if (edgeIndex === -1) {
    ...
    const generateEdgeSlug = useCallback((sourceId, targetId) => {
        if (!graph?.nodes)
    ...
    const onConnect = useCallback((connection) => {
        ...
        if (!isCaseNode) {
            ...
            const existingEdge = graph.edges.find(edge => edge.from === connection.source && edge.to === connection.target);
            if (existingEdge) {
    ...
    const [caseNodeVariants, setCaseNodeVariants] = useState([]);
    // Handle variant selection for case edges
    const handleVariantSelection = useCallback((variant) => {
        if (!pendingConnection || !graph)
            return;
    ...
    useEffect(() => {
        ...
        const handleMouseDown = (e) => {
            if (isShiftHeld && e.target && e.target.closest('.react-flow')) {
                e.preventDefault();
                e.stopPropagation();
    ...
    const [selectedNodesForAnalysis, setSelectedNodesForAnalysis] = useState([]);
    // Helper function to find start nodes (nodes with no incoming edges)
    const findStartNodes = useCallback((allNodes, allEdges) => {
    ...
    const findAllPaths = useCallback((sourceId, targetId, allEdges, maxDepth = 10) => {
        const paths = [];
        const visited = new Set();
        const dfs = (currentNodeId, currentPath, depth) => {
    ...
    const topologicalSort = useCallback((nodeIds, allEdges) => {
    ...
    const areNodesTopologicallySequential = useCallback((sortedNodeIds, allEdges) => {
    ...
    const findPathEdges = useCallback((selectedNodes, allEdges) => {
        ...
        if (selectedNodes.length === 1) {
            ...
            const findUpstreamEdges = (nodeId, depth, visited = new Set()) => {
            ...
            const findDownstreamEdges = (nodeId, depth, visited = new Set()) => {
                if (visited.has(nodeId) || depth > 5)
                    return;
                visited.add(nodeId);
                allEdges.forEach(edge => {
        ...
        if (selectedNodes.length >= 3) {
            ...
            if (isSequential) {
                ...
                const findPathsThroughNodes = (currentId, remainingNodes, currentPath) => {
    ...
    useEffect(() => {
        setEdges(prevEdges => {
            ...
            if (selectedNodesForAnalysis.length === 1) {
                ...
                const calculateUpstreamDepths = (nodeId, depth, visited = new Set()) => {
                    if (visited.has(nodeId) || depth > 5)
                        return;
                    visited.add(nodeId);
                ...
                const calculateDownstreamDepths = (nodeId, depth, visited = new Set()) => {
                    if (visited.has(nodeId) || depth > 5)
                        return;
                    visited.add(nodeId);
                    prevEdges.forEach(edge => {
    ...
    const calculateSelectionAnalysis = useCallback(() => {
        ...
        const findPathThroughIntermediates = (startId, endId, givenVisitedNodeIds, prunedEdges, renormFactors) => {
            const visited = new Set();
            const costs = {};
            // Get current state from store (avoid stale closures)
            const currentGraph = useGraphStore.getState().graph;
            ...
            const excludedEdges = prunedEdges || new Set();
            const edgeRenormalizationFactors = renormFactors || new Map();
            // Build set of nodes guaranteed to be visited in this path context
            const givenNodesSet = givenVisitedNodeIds ? new Set(givenVisitedNodeIds) : new Set();
            const dfs = (nodeId, currentPathContext) => {
            ...
            const probVisited = new Set();
            const calculateProbability = (nodeId, currentPathContext) => {
                if (nodeId === endId)
                    return 1;
        ...
        if (selectedNodesForAnalysis.length === 2) {
            ...
            const directEdge = edges.find(edge => edge.source === nodeA.id && edge.target === nodeB.id);
            // Find reverse edge (B → A) 
            const reverseEdge = edges.find(edge => edge.source === nodeB.id && edge.target === nodeA.id);
        ...
        const computeGlobalPruning = (pathStart, pathEnd, allSelectedIds) => {
            const excludedEdges = new Set();
            const renormFactors = new Map();
            const impliedCaseOverrides = new Map(); // case node ID → forced variant
            const currentGraph = useGraphStore.getState().graph;
            const currentOverrides = useGraphStore.getState().whatIfOverrides;
            const currentWhatIfAnalysis = useGraphStore.getState().whatIfAnalysis;
        ...
        if (selectedNodesForAnalysis.length >= 3) {
            ...
            if (isSequential || hasUniqueStartEnd) {
                ...
                const pathAnalysis = findPathThroughIntermediates(firstNodeId, lastNodeId, sortedNodeIds, excludedEdges, renormFactors);
        ...
        const internalEdges = edges.filter(edge => selectedNodeIds.includes(edge.source) && selectedNodeIds.includes(edge.target));
        const incomingEdges = edges.filter(edge => !selectedNodeIds.includes(edge.source) && selectedNodeIds.includes(edge.target));
        const outgoingEdges = edges.filter(edge => selectedNodeIds.includes(edge.source) && !selectedNodeIds.includes(edge.target));
    ...
    const performAutoLayout = useCallback(() => {
        ...
        dagreGraph.nodes().forEach((nodeId) => {
            ...
            const graphNode = nextGraph.nodes.find((n) => n.id === nodeId);
            if (graphNode) {
                if (!graphNode.layout)
                    graphNode.layout = {};
    ...
    if (!graph) {
        return _jsx("div", { style: { display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100%' }, children: "Loading..." });
    }
    return (_jsxs("div", { style: { height: '100%', position: 'relative' }, children: [_jsxs(ReactFlow, { nodes: nodes, edges: edges, onNodesChange: onNodesChange, onEdgesChange: onEdgesChange, onConnect: onConnect, onReconnect: onEdgeUpdate, onSelectionChange: onSelectionChange, nodeTypes: nodeTypes, edgeTypes: edgeTypes, connectionMode: ConnectionMode.Loose, fitView: true, selectionOnDrag: false, selectNodesOnDrag: false, selectionKeyCode: ['Meta', 'Ctrl'], nodesDraggable: true, nodesConnectable: true, elementsSelectable: true, reconnectRadius: 50, edgeUpdaterRadius: 80, onlyRenderVisibleElements: false, panOnDrag: !isLassoSelecting, connectionRadius: 50, snapToGrid: false, snapGrid: [1, 1], style: { background: '#f8f9fa' }, children: [_jsx(Background, {}), _jsx(Controls, {}), _jsx(MiniMap, {}), isLassoSelecting && lassoStart && lassoEnd && (_jsx("div", { style: {
                            position: 'absolute',
                            ...
                            pointerEvents: 'none',
                            zIndex: 1000,
                        } })), _jsxs(Panel, { position: "top-left", children: [_jsx("button", { onClick: addNode, style: {
                                    padding: '8px 16px',
                                    background: '#007bff',
                                    color: 'white',
                                    border: 'none',
                                    ...
                                    cursor: 'pointer',
                                    marginRight: '8px',
                                }, children: "+ Add Node" }), _jsx("button", { onClick: deleteSelected, style: {
                                    padding: '8px 16px',
                                    background: '#dc3545',
                                    color: 'white',
                                    border: 'none',
                                    ...
                                    cursor: 'pointer',
                                    marginRight: '8px',
                                }, children: "Delete Selected" }), _jsxs("select", { value: layoutDirection, onChange: (e) => setLayoutDirection(e.target.value), style: {
                                    padding: '8px 12px',
                                    ...
                                    marginRight: '8px',
                                    fontSize: '14px',
                                }, title: "Layout direction", children: [_jsx("option", { value: "LR", children: "Left \u2192 Right" }), _jsx("option", { value: "RL", children: "Right \u2192 Left" }), _jsx("option", { value: "TB", children: "Top \u2192 Bottom" }), _jsx("option", { value: "BT", children: "Bottom \u2192 Top" })] }), _jsx("button", { onClick: performAutoLayout, style: {
                                    padding: '8px 16px',
                                    background: '#28a745',
                                    color: 'white',
                                    border: 'none',
                                    borderRadius: '4px',
                                    cursor: 'pointer',
                                }, title: nodes.filter(n => n.selected).length > 0 ? "Auto-layout selected nodes" : "Auto-layout all nodes", children: "\uD83D\uDD27 Auto Layout" })] }), showLayoutModal && (_jsx("div", { style: {
                            position: 'absolute',
                            top: 0,
                            left: 0,
                            right: 0,
                            ...
                            zIndex: 10000,
                                ...
                                boxShadow: '0 4px 24px rgba(0, 0, 0, 0.3)',
                            }, children: [_jsx("h3", { style: { margin: '0 0 16px 0', fontSize: '18px' }, children: "Keep new layout?" }), _jsx("p", { style: { margin: '0 0 24px 0', color: '#666', fontSize: '14px' }, children: "The graph has been automatically laid out. Would you like to keep this new layout or revert to the previous one?" }), _jsxs("div", { style: { display: 'flex', gap: '12px', justifyContent: 'flex-end' }, children: [_jsx("button", { onClick: handleRevertLayout, style: {
                                                padding: '10px 20px',
                                                background: '#6c757d',
                                                color: 'white',
                                                border: 'none',
                                                ...
                                                cursor: 'pointer',
                                                fontSize: '14px',
                                            }, children: "Revert" }), _jsx("button", { onClick: handleKeepLayout, style: {
                                                padding: '10px 20px',
                                                background: '#28a745',
                                                color: 'white',
                                                border: 'none',
                                                ...
                                                cursor: 'pointer',
                                                fontSize: '14px',
                                            }, children: "Keep Layout" })] })] }) })), analysis && (_jsx(Panel, { position: "bottom-left", children: _jsxs("div", { style: {
                                background: 'white',
                                ...
                                fontSize: '14px',
                                lineHeight: '1.4'
                            }, children: [_jsx("h3", { style: { margin: '0 0 12px 0', color: '#007bff', fontSize: '16px' }, children: analysis.type === 'path' || analysis.type === 'path_sequential' || analysis.type === 'single' ? 'Path Analysis' : 'Selection Analysis' }), analysis.type === 'single' ? (
                                // Path analysis for 1 node (Start to selected)
                                _jsx(_Fragment, { children: analysis.error ? (_jsxs("div", { style: { color: '#ff6b6b', fontSize: '12px' }, children: ["\u26A0\uFE0F ", analysis.error] })) : analysis.isStartNode ? (_jsxs(_Fragment, { children: [_jsxs("div", { style: { marginBottom: '12px' }, children: [_jsx("strong", { children: "Node:" }), " ", analysis.node.data?.label || analysis.node.id] }), _jsx("div", { style: { color: '#16a34a', fontSize: '12px' }, children: "\u2705 This is the start node" })] })) : (_jsxs(_Fragment, { children: [_jsxs("div", { style: { marginBottom: '12px' }, children: [_jsx("strong", { children: "Path:" }), " ", analysis.startNode?.data?.label || analysis.startNode?.id || 'Start', " \u2192 ", analysis.node.data?.label || analysis.node.id] }), (analysis.pathProbability !== undefined && analysis.pathProbability > 0) ? (_jsxs(_Fragment, { children: [_jsxs("div", { style: { marginBottom: '8px' }, children: [_jsx("strong", { children: "Probability:" }), " ", (analysis.pathProbability * 100).toFixed(2), "%"] }), ((analysis.pathCosts?.monetary || 0) > 0 || (analysis.pathCosts?.time || 0) > 0) && (_jsxs("div", { style: { marginBottom: '8px' }, children: [_jsx("strong", { children: "Expected Cost (Given Path):" }), _jsxs("div", { style: { marginLeft: '12px', fontSize: '12px' }, children: [(analysis.pathCosts?.monetary || 0) > 0 && (_jsxs("div", { children: ["\uD83D\uDCB0 \u00A3", (analysis.pathCosts?.monetary || 0).toFixed(2), " per conversion"] })), (analysis.pathCosts?.time || 0) > 0 && (_jsxs("div", { children: ["\u23F1\uFE0F ", (analysis.pathCosts?.time || 0).toFixed(1), " ", analysis.pathCosts?.units || 'units', " per conversion"] }))] })] }))] })) : (_jsx("div", { style: { color: '#ff6b6b', fontSize: '12px' }, children: "\u26A0\uFE0F No path found from start" }))] })) })) : analysis.type === 'path_sequential' ? (
                                // Path analysis for 3+ topologically sequential nodes
                                _jsxs(_Fragment, { children: [_jsxs("div", { style: { marginBottom: '12px' }, children: [_jsx("strong", { children: "Path:" }), " ", analysis.nodeA?.data?.label || analysis.nodeA?.id, " \u2192 ", analysis.nodeB?.data?.label || analysis.nodeB?.id, analysis.intermediateNodes && analysis.intermediateNodes.length > 0 && (_jsxs("div", { style: { color: '#666', fontSize: '12px', marginTop: '2px' }, children: ["via ", analysis.intermediateNodes.map((n) => n?.data?.label || n?.id).join(' → ')] }))] }), (analysis.pathProbability !== undefined && analysis.pathProbability > 0) ? (_jsxs(_Fragment, { children: [_jsxs("div", { style: { marginBottom: '8px' }, children: [_jsx("strong", { children: "Probability:" }), " ", (analysis.pathProbability * 100).toFixed(2), "%"] }), ((analysis.pathCosts?.monetary || 0) > 0 || (analysis.pathCosts?.time || 0) > 0) && (_jsxs("div", { style: { marginBottom: '8px' }, children: [_jsx("strong", { children: "Expected Cost (Given Path):" }), _jsxs("div", { style: { marginLeft: '12px', fontSize: '12px' }, children: [(analysis.pathCosts?.monetary || 0) > 0 && (_jsxs("div", { children: ["\uD83D\uDCB0 \u00A3", (analysis.pathCosts?.monetary || 0).toFixed(2), " per conversion"] })), (analysis.pathCosts?.time || 0) > 0 && (_jsxs("div", { children: ["\u23F1\uFE0F ", (analysis.pathCosts?.time || 0).toFixed(1), " ", analysis.pathCosts?.units || 'units', " per conversion"] }))] })] }))] })) : (_jsx("div", { style: { color: '#ff6b6b', fontSize: '12px' }, children: "\u26A0\uFE0F No connection found through selected nodes" }))] })) : analysis.type === 'path' ? (
                                // Path analysis for exactly 2 nodes
                                _jsxs(_Fragment, { children: [_jsxs("div", { style: { marginBottom: '12px' }, children: [_jsx("strong", { children: "Path:" }), " ", analysis.nodeA.data?.label || analysis.nodeA.id, " \u2192 ", analysis.nodeB.data?.label || analysis.nodeB.id] }), (analysis.pathProbability !== undefined && analysis.pathProbability > 0) ? (_jsxs(_Fragment, { children: [_jsxs("div", { style: { marginBottom: '8px' }, children: [_jsx("strong", { children: "Probability:" }), " ", (analysis.pathProbability * 100).toFixed(2), "%", !analysis.isDirectPath && (analysis.intermediateNodes?.length || 0) > 0 && (_jsxs("div", { style: { color: '#666', fontSize: '12px', marginTop: '2px' }, children: ["via ", analysis.intermediateNodes?.length || 0, " intermediate node", (analysis.intermediateNodes?.length || 0) > 1 ? 's' : ''] }))] }), ((analysis.pathCosts?.monetary || 0) > 0 || (analysis.pathCosts?.time || 0) > 0) && (_jsxs("div", { style: { marginBottom: '8px' }, children: [_jsx("strong", { children: "Expected Cost (Given Path):" }), _jsxs("div", { style: { marginLeft: '12px', fontSize: '12px' }, children: [(analysis.pathCosts?.monetary || 0) > 0 && (_jsxs("div", { children: ["\uD83D\uDCB0 \u00A3", (analysis.pathCosts?.monetary || 0).toFixed(2), " per conversion"] })), (analysis.pathCosts?.time || 0) > 0 && (_jsxs("div", { children: ["\u23F1\uFE0F ", (analysis.pathCosts?.time || 0).toFixed(1), " ", analysis.pathCosts?.units || 'units', " per conversion"] }))] })] })), analysis.hasReversePath && (_jsx("div", { style: { color: '#666', fontSize: '12px', marginTop: '8px' }, children: "\u2139\uFE0F Bidirectional path exists" }))] })) : (_jsx("div", { style: { color: '#ff6b6b', fontSize: '12px' }, children: "\u26A0\uFE0F No connection found (direct or via intermediates)" }))] })) : (
                                // Multi-node analysis (existing logic)
                                _jsxs(_Fragment, { children: [_jsxs("div", { style: { marginBottom: '8px' }, children: [_jsx("strong", { children: "Nodes:" }), " ", analysis.selectedNodes, " selected"] }), _jsxs("div", { style: { marginBottom: '8px' }, children: [_jsx("strong", { children: "Edges:" }), " ", analysis.internalEdges, " internal, ", analysis.incomingEdges, " incoming, ", analysis.outgoingEdges, " outgoing"] }), _jsxs("div", { style: { marginBottom: '8px' }, children: [_jsx("strong", { children: "Probability Flow:" }), _jsxs("div", { style: { marginLeft: '12px', fontSize: '12px' }, children: ["In: ", Math.round((analysis.totalIncomingProbability || 0) * 100), "% \u2192 Out: ", Math.round((analysis.totalOutgoingProbability || 0) * 100), "%"] }), (analysis.totalOutgoingProbability || 0) === 0 && (analysis.totalIncomingProbability || 0) > 0 ? (_jsx("div", { style: { color: '#16a34a', fontSize: '12px', marginTop: '4px' }, children: "\u2705 Complete path selected - probability contained within selection" })) : !analysis.probabilityConservation ? (_jsx("div", { style: { color: '#ff6b6b', fontSize: '12px', marginTop: '4px' }, children: "\u26A0\uFE0F Probability not conserved" })) : (_jsx("div", { style: { color: '#16a34a', fontSize: '12px', marginTop: '4px' }, children: "\u2705 Probability conserved" }))] }), ((analysis.totalCosts?.monetary || 0) > 0 || (analysis.totalCosts?.time || 0) > 0) && (_jsxs("div", { style: { marginBottom: '8px' }, children: [_jsx("strong", { children: "Total Costs:" }), _jsxs("div", { style: { marginLeft: '12px', fontSize: '12px' }, children: [(analysis.totalCosts?.monetary || 0) > 0 && (_jsxs("div", { children: ["\u00A3", analysis.totalCosts?.monetary, analysis.totalCosts?.units && ` ${analysis.totalCosts.units}`] })), (analysis.totalCosts?.time || 0) > 0 && (_jsxs("div", { children: [analysis.totalCosts?.time, "h", analysis.totalCosts?.units && ` ${analysis.totalCosts.units}`] }))] })] }))] }))] }) }))] }), showVariantModal && (_jsx("div", { style: {
                    position: 'fixed',
                    top: 0,
                    left: 0,
                    right: 0,
                    ...
                    zIndex: 1000
                        ...
                        maxWidth: '400px',
                        width: '90%'
                    }, children: [_jsx("h3", { style: { margin: '0 0 16px 0', fontSize: '18px', fontWeight: '600' }, children: "Select Variant for Case Edge" }), _jsx("p", { style: { margin: '0 0 16px 0', color: '#666', fontSize: '14px' }, children: "Choose which variant this edge represents:" }), _jsx("div", { style: { marginBottom: '16px' }, children: caseNodeVariants.map((variant, index) => {
                                // Check if this variant already has an edge to the target
                                const sourceNode = graph?.nodes.find(n => n.id === pendingConnection?.source);
                                const hasExistingEdge = graph?.edges.some(edge => edge.from === pendingConnection?.source &&
                                    edge.to === pendingConnection?.target &&
                                    edge.case_id === sourceNode?.case?.id &&
                                    edge.case_variant === variant.name);
                                return (_jsxs("button", { onClick: () => handleVariantSelection(variant), disabled: hasExistingEdge, style: {
                                        width: '100%',
                                        padding: '12px 16px',
                                        marginBottom: '8px',
                                    ...
                                    }, children: [_jsxs("div", { style: { fontWeight: '600', marginBottom: '4px' }, children: [variant.name, hasExistingEdge && _jsx("span", { style: { color: '#666', fontWeight: 'normal', marginLeft: '8px' }, children: "\u2713 Already connected" })] }), _jsxs("div", { style: { color: '#666', fontSize: '12px' }, children: ["Weight: ", (variant.weight * 100).toFixed(0), "%", variant.description && ` • ${variant.description}`] })] }, index));
                            }) }), _jsx("button", { onClick: () => {
                                setShowVariantModal(false);
                                setPendingConnection(null);
                                setCaseNodeVariants([]);
