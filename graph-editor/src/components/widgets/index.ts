/**
 * Custom Widgets for React JSON Schema Form (@rjsf/mui)
 * 
 * These widgets enhance FormEditor with rich editing capabilities
 */

export {
  MonacoWidget,
  createMonacoWidget,
  JsonMonacoWidget,
  YamlMonacoWidget,
  JavaScriptMonacoWidget,
  JMESPathMonacoWidget,
  JSONataMonacoWidget
} from './MonacoWidget';

export { TabbedArrayWidget } from './TabbedArrayWidget';
export { AccordionObjectFieldTemplate } from './AccordionObjectFieldTemplate';

// Future widgets will be exported here:
// export { ConnectionSelectorWidget } from './ConnectionSelectorWidget';

